...
public abstract partial class CSceneManager : CComponent {
	...
	public Camera UIsCamera => m_oCameraDict[EKey.UIS_CAMERA];
	...
	public GameObject UIsTop => m_oUIsDict[EKey.UIS_TOP];
	...
	public static Camera ActiveSceneUIsCamera => CSceneManager.m_oActiveSceneCameraDict[EKey.UIS_CAMERA];
	...
	public static GameObject ActiveSceneUIsTop => CSceneManager.m_oActiveSceneUIsDict[EKey.UIS_TOP];
	...
	public override void Awake() {
		...
		try {
...
#if (DEBUG || DEVELOPMENT_BUILD || PLAY_TEST_ENABLE) && (!ROBO_TEST_ENABLE && !STORE_DIST_BUILD && !EDITOR_DIST_BUILD && !CREATIVE_DIST_BUILD)
				this.TestUIs?.SetActive(true);
#else
				this.TestUIs?.SetActive(false);
#endif			// #if (DEBUG || DEVELOPMENT_BUILD || PLAY_TEST_ENABLE) && (!ROBO_TEST_ENABLE && !STORE_DIST_BUILD && !EDITOR_DIST_BUILD && !CREATIVE_DIST_BUILD)
			}
		} finally {
			oStopwatch.Stop();
			...
			if(this.IsActiveScene) {
				CSceneManager.SetStaticDebugStr($"{this.SceneName}.SetupScene(true): {oStopwatch.ElapsedMilliseconds} ms");
			} else {
				CSceneManager.AddStaticDebugStr($"{this.SceneName}.SetupScene(true): {oStopwatch.ElapsedMilliseconds} ms");
			}
		}
	}
	...
	public override void Start() {
		...
		try {
			...
			oStopwatch.Stop();
			CSceneManager.AddStaticDebugStr($"{this.SceneName}.SetupScene(false): {oStopwatch.ElapsedMilliseconds} ms\n");
		}
	}
		...
		try {
			...
			if(CSceneManager.IsAwake || CSceneManager.IsAppRunning) {
				...
				if(this.IsActiveScene) {
					...
					if(this.UIsCamera != null && CSceneManager.m_oActiveSceneCameraDataDict[EKey.MAIN_CAMERA_DATA] != null && QualitySettings.renderPipeline != null && GraphicsSettings.renderPipelineAsset != null) {
						CSceneManager.m_oActiveSceneCameraDataDict[EKey.MAIN_CAMERA_DATA].cameraStack?.ExRemoveVal(this.UIsCamera, false);
					}
#endif			// #if UNIVERSAL_RENDERING_PIPELINE_MODULE_ENABLE
				}
