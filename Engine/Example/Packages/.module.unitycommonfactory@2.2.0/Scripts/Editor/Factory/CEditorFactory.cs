...
public static class CEditorFactory {
	...
	public static void MakeDirs(string a_oDirPath, bool a_bIsEnableAssert = true) {
		...
		if(a_oDirPath.ExIsValid() && a_oDirPath.Contains(KCEditorDefine.B_DIR_P_ASSETS)) {
			var oTokens = a_oDirPath.Split(KCDefine.B_TOKEN_SPLASH);
			var oStrBuilder = new System.Text.StringBuilder();

			for(int i = 1; i < oTokens.Length; ++i) {
				oStrBuilder.AppendFormat(KCDefine.B_TEXT_FMT_2_COMBINE, (i > KCDefine.B_VAL_1_INT) ? KCDefine.B_TOKEN_SPLASH : string.Empty, oTokens[i - KCDefine.B_VAL_1_INT]);
