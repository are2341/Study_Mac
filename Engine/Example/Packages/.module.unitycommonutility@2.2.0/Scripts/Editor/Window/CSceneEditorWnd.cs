	...
	private void DrawFontEditorGUI() {
		...
		if(GUILayout.Button(KCEditorDefine.B_TEXT_APPLY, GUILayout.Width(this.WndWidth)) && m_oStrDict[EKey.REPLACE_FONT_PATH].ExIsValid()) {
			...
			for(int i = 0; i < oTMPTextList.Count; ++i) {
				oTMPTextList[i].font = (oTMPTextList[i].font == null || (m_oStrDict[EKey.TARGET_FONT_NAME].Length <= KCDefine.B_VAL_0_INT || m_oStrDict[EKey.TARGET_FONT_NAME].Equals(oTextList[i].font.name))) ? oFontAsset : oTMPTextList[i].font;
