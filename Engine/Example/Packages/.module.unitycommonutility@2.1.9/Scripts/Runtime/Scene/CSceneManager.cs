...
public abstract partial class CSceneManager : CComponent {
	...
	#region 클래스 함수
	/** 터치 응답자를 출력한다 */
	public static void ShowTouchResponder(GameObject a_oParent, string a_oKey, string a_oObjPath, Color a_stColor, System.Action<GameObject> a_oCallback, bool a_bIsEnableNavStack = false, float a_fDuration = KCDefine.B_VAL_0_FLT, System.Func<GameObject, string, Color, float, Tween> a_oAniCreator = null) {
		// 터치 응답자가 없을 경우
		...
		if(CSceneManager.m_oTouchResponderInfoDict.TryGetValue(a_oKey, out STTouchResponderInfo stTouchResponderInfo)) {
			...
			if(a_oAniCreator != null) {
				CFactory.MakeSequence(a_oAniCreator(stTouchResponderInfo.m_oTouchResponder, a_oKey, a_stColor, a_fDuration), (a_oSender) => {
					a_oSender.Kill();
					CFunc.Invoke(ref a_oCallback, stTouchResponderInfo.m_oTouchResponder);
	...
	private void UpdateDebugUIsState(float a_fDeltaTime) {
		...
		if(CSceneManager.m_fDebugInfoSkipTime.ExIsGreateEquals(KCDefine.B_VAL_1_FLT)) {
			...
			CSceneManager.m_oDynamicDebugStrBuilder.AppendFormat(KCDefine.U_TEXT_FMT_DYNAMIC_DEBUG_INFO_01, Profiler.usedHeapSizeLong.ExByteToMegaByte(), Profiler.GetAllocatedMemoryForGraphicsDriver().ExByteToMegaByte());
			CSceneManager.m_oDynamicDebugStrBuilder.AppendFormat(KCDefine.U_TEXT_FMT_DYNAMIC_DEBUG_INFO_02, Profiler.GetMonoHeapSizeLong().ExByteToMegaByte(), Profiler.GetMonoUsedSizeLong().ExByteToMegaByte());
			CSceneManager.m_oDynamicDebugStrBuilder.AppendFormat(KCDefine.U_TEXT_FMT_DYNAMIC_DEBUG_INFO_03, Profiler.GetTempAllocatorSize().ExByteToMegaByte(), Profiler.GetTotalAllocatedMemoryLong().ExByteToMegaByte());
			CSceneManager.m_oDynamicDebugStrBuilder.AppendFormat(KCDefine.U_TEXT_FMT_DYNAMIC_DEBUG_INFO_04, Profiler.GetTotalReservedMemoryLong().ExByteToMegaByte(), Profiler.GetTotalUnusedReservedMemoryLong().ExByteToMegaByte());
			CSceneManager.m_oDynamicDebugStrBuilder.AppendFormat(KCDefine.U_TEXT_FMT_DYNAMIC_DEBUG_INFO_05, Time.timeScale);
			
			CSceneManager.ScreenStaticDebugText?.ExSetText<TMP_Text>(string.Format(KCDefine.U_TEXT_FMT_STATIC_DEBUG_MSG, CSceneManager.m_oStaticDebugStrBuilder.ToString(), CSceneManager.m_oExtraStaticDebugStrBuilder.ToString()));
			CSceneManager.ScreenDynamicDebugText?.ExSetText<TMP_Text>(string.Format(KCDefine.U_TEXT_FMT_DYNAMIC_DEBUG_MSG, CSceneManager.m_oDynamicDebugStrBuilder.ToString(), CSceneManager.m_oExtraDynamicDebugStrBuilder.ToString()));
