...
public abstract partial class CSceneManager : CComponent {
	...
	private void SetupNonAnchorUIsState(GameObject a_oUIsObjs, ref Vector3 a_rstPos, ref Vector3 a_rstSize, ref Vector3 a_rstAnchorMin, ref Vector3 a_rstAnchorMax, bool a_bIsEnableAssert = true) {
		...
		if(a_oUIsObjs != null) {
			...
			if(bIsUIs || bIsTestUIs) {
				a_rstSize = KCDefine.B_SCREEN_SIZE;
			}
			// 크기 보정 UI 일 경우
	...
	private void SetupDebugUIsState(GameObject a_oUIsObjs, bool a_bIsEnableAssert = true) {
		...
		if(a_oUIsObjs != null && CSceneManager.ScreenStaticDebugText != null) {
			...
			CSceneManager.m_oStaticDebugStrBuilder.AppendFormat(KCDefine.U_TEXT_FMT_STATIC_DEBUG_INFO_01, CSceneManager.UpOffset, CSceneManager.DownOffset, CSceneManager.LeftOffset, CSceneManager.RightOffset);
			CSceneManager.m_oStaticDebugStrBuilder.AppendFormat(KCDefine.U_TEXT_FMT_STATIC_DEBUG_INFO_02, CSceneManager.UpRootOffset, CSceneManager.DownRootOffset, CSceneManager.LeftRootOffset, CSceneManager.RightRootOffset);
			CSceneManager.m_oStaticDebugStrBuilder.AppendFormat(KCDefine.U_TEXT_FMT_STATIC_DEBUG_INFO_03, CAccess.ScreenSize.x, CAccess.ScreenSize.y);

#if ADS_MODULE_ENABLE
			CSceneManager.m_oStaticDebugStrBuilder.AppendFormat(KCDefine.U_TEXT_FMT_STATIC_DEBUG_INFO_04, CAccess.ScreenDPI, CAccess.GetBannerAdsHeight(KCDefine.U_SIZE_BANNER_ADS.y));
#endif			// #if ADS_MODULE_ENABLE

			CSceneManager.m_oStaticDebugStrBuilder.AppendFormat(KCDefine.U_TEXT_FMT_STATIC_DEBUG_INFO_05, (EQualityLevel)QualitySettings.GetQualityLevel(), Application.targetFrameRate);
			CSceneManager.m_oStaticDebugStrBuilder.AppendFormat(KCDefine.U_TEXT_FMT_STATIC_DEBUG_INFO_06, KCDefine.B_DIR_P_WRITABLE);
		}
	}
#endif			// #if DEBUG || DEVELOPMENT_BUILD
