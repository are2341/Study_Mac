...
public class CValTable : CSingleton<CValTable> {
	#region 프로퍼티
	public override bool IsIgnoreIntDict => false;
	public override bool IsIgnoreFltDict => false;
	public override bool IsIgnoreStrDict => false;
	#endregion			// 프로퍼티
	...
	public float GetFlt(string a_oKey, float a_fDefVal = KCDefine.B_VAL_0_FLT) {
		CAccess.Assert(a_oKey.ExIsValid());
		return m_oFltDict.GetValueOrDefault(a_oKey, a_fDefVal);
	}
	...
	public void AddFlt(string a_oKey, float a_fVal, bool a_bIsReplace = false) {
		...
		// 실수 추가가 가능 할 경우
		if(a_bIsReplace || !m_oFltDict.ContainsKey(a_oKey)) {
			m_oFltDict.ExReplaceVal(a_oKey, a_fVal);
		}
	}
