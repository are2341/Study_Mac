...
public partial class CLocalizeInfoTable : CScriptableObj<CLocalizeInfoTable> {
	...
	public bool TryGetLocalizeInfo(string a_oCountryCode, SystemLanguage a_eSystemLanguage, out STLocalizeInfo a_stOutLocalizeInfo) {
		int nIdxA = m_oLocalizeInfoList.FindIndex((a_stLocalizeInfo) => a_stLocalizeInfo.m_eSystemLanguage == a_eSystemLanguage);
		int nIdxB = m_oLocalizeInfoList.FindIndex((a_stLocalizeInfo) => a_stLocalizeInfo.m_oCountryCode.ExIsValid() && a_stLocalizeInfo.m_oCountryCode.Equals(a_oCountryCode));
				
		a_stOutLocalizeInfo = m_oLocalizeInfoList.ExIsValidIdx(nIdxA) ? m_oLocalizeInfoList[nIdxA] : m_oLocalizeInfoList.ExIsValidIdx(nIdxB) ? m_oLocalizeInfoList[nIdxB] : KCDefine.U_INVALID_LOCALIZE_INFO;
		return m_oLocalizeInfoList.ExIsValidIdx(nIdxA) || m_oLocalizeInfoList.ExIsValidIdx(nIdxB);
	}
	...
	public bool TryGetFontSetInfo(string a_oCountryCode, SystemLanguage a_eSystemLanguage, EFontSet a_eFontSet, out STFontSetInfo a_stOutFontSetInfo) {
		...
		if(this.TryGetLocalizeInfo(a_oCountryCode, a_eSystemLanguage, out STLocalizeInfo stLocalizeInfo)) {
			...
			a_stOutFontSetInfo = stLocalizeInfo.m_oFontSetInfoList.ExIsValidIdx(nIdx) ? stLocalizeInfo.m_oFontSetInfoList[nIdx] : KCDefine.U_INVALID_FONT_SET_INFO;
