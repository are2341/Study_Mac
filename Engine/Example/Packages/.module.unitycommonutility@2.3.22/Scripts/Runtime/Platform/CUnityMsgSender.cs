...
public partial class CUnityMsgSender : CSingleton<CUnityMsgSender> {
...
#if (UNITY_IOS || (UNITY_ANDROID && GOOGLE_PLAY_UPDATE_ENABLE)) && (STORE_VER_CHECK_ENABLE && NEWTON_SOFT_JSON_MODULE_ENABLE)
#if UNITY_IOS
...
#endif			// #if (UNITY_IOS || (UNITY_ANDROID && GOOGLE_PLAY_UPDATE_ENABLE)) && (STORE_VER_CHECK_ENABLE && NEWTON_SOFT_JSON_MODULE_ENABLE)
	}
	...
	public void SendMailMsg(string a_oRecipient, string a_oTitle, string a_oMsg) {
		...
		string oMailMsg = string.Format(KCDefine.B_MAIL_MSG_FMT, CProjInfoTable.Inst.CommonProjInfo.m_oShortProductName, oVer, CCommonAppInfoStorage.Inst.Platform, SystemInfo.processorType, SystemInfo.graphicsDeviceName, SystemInfo.graphicsDeviceType, SystemInfo.operatingSystem, CCommonAppInfoStorage.Inst.AppInfo.DeviceID, a_oMsg);
	...
	private void HandleShareMsg(NativeShare.ShareResult a_eResult, string a_oTarget) {
		...
		CScheduleManager.Inst.AddCallback(KCDefine.U_KEY_UNITY_MS_SHARE_MSG_CALLBACK, () => m_oCallbackDict.GetValueOrDefault(ECallback.SHARE)?.Invoke(a_eResult, a_oTarget));
	}
...
#endif			// #if UNITY_IOS
#else
		this.ExLateCallFunc((a_oSender) => CFunc.Invoke(ref a_oCallback, a_oCmd, string.Empty));
