...
public class CMenuSceneManager : CSceneManager {
	...
	#region 클래스 변수
	protected static Vector3 m_stPrevScrollPos = Vector3.up;
	#endregion			// 클래스 변수
	...
	private void SetupAwake() {
		...
		for(int i = KSDefine.MS_DEF_SCENE_NAMES.Count; i < SceneManager.sceneCountInBuildSettings; ++i) {
			...
			// 텍스트를 설정한다 {
			var oText = CFactory.CreateCloneObj<TMP_Text>(string.Format(KSDefine.MS_OBJ_N_FMT_TEXT, i), KSDefine.MS_OBJ_P_BTN, oContents);
		...
		for(int i = 0; i < KCDefine.B_UNIT_DIGITS_PER_TEN; ++i) {
			m_oEmptyTextList.Add(CFactory.CreateCloneObj<TMP_Text>(KCDefine.U_OBJ_N_EMPTY, KSDefine.MS_OBJ_P_BTN, oContents));
		}
	}
	...
	private void SetupStart() {
		...
		this.ExLateCallFunc((a_oSender) => m_oScrollRect.normalizedPosition = CMenuSceneManager.m_stPrevScrollPos);
	}
	
	/** 버튼을 눌렀을 경우 */
	private void OnTouchBtn(int a_nIdx) {
		CMenuSceneManager.m_stPrevScrollPos = m_oScrollRect.normalizedPosition;
