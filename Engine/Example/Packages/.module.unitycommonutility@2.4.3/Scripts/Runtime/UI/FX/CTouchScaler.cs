...
public partial class CTouchScaler : CComponent, IPointerUpHandler, IPointerDownHandler {
	...
	public virtual void ResetAni() {
		m_oAniDict[EKey.SCALE_ANI]?.Kill();
	}
	...
	public void SetOriginScale(Vector3 a_stScale) {
		m_oVec3Dict[EKey.ORIGIN_SCALE] = a_stScale;
	}
	...
	public void SetScale(Vector3 a_stScale, bool a_bIsAni = true) {
		...
		if(!this.IsIgnoreAni && a_bIsAni) {
			m_oAniDict[EKey.SCALE_ANI] = this.transform.DOScale(a_stScale, m_fDuration).SetAutoKill().SetEase(KCDefine.U_EASE_LINEAR).SetUpdate(true);
		} else {
	...
	private void StartPressAni() {
		this.SetScale(m_oVec3Dict[EKey.ORIGIN_SCALE] * m_fTouchScale);
	}

	/** 누르기 종료 애니메이션을 시작한다 */
	private void StartReleaseAni() {
		this.SetScale(m_oVec3Dict[EKey.ORIGIN_SCALE]);
	}
	#endregion			// 함수
}
