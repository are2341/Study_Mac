	...
	protected virtual void SetupScene(bool a_bIsMainSetup) {
		...
		if(a_bIsMainSetup) {
			...
			if(this.MainCamera != null) {
				this.MainCamera.clearFlags = !this.IsActiveScene ? CameraClearFlags.Nothing : (this.MainCameraProjection == EProjection._2D) ? CameraClearFlags.SolidColor : CameraClearFlags.Skybox;
	...
	protected virtual void SetupLights() {
		...
		if(this.IsActiveScene) {
			...
			var oIsSetupOptsList = new List<bool>() {
				...
				RenderSettings.ambientIntensity.ExIsEquals(KCDefine.B_VAL_1_FLT),
				RenderSettings.reflectionIntensity.ExIsEquals(KCDefine.B_VAL_1_FLT),
				RenderSettings.subtractiveShadowColor.ExIsEquals(Color.white)
			};

			// 설정 갱신이 필요 할 경우
			if(oIsSetupOptsList.Contains(false)) {
				RenderSettings.skybox = RenderSettings.skybox ?? Resources.Load<Material>(KCDefine.U_MAT_P_G_SKYBOX);
				RenderSettings.ambientMode = AmbientMode.Skybox;
				RenderSettings.defaultReflectionMode = DefaultReflectionMode.Skybox;
	...
	private void SetupBlindUIs(GameObject a_oUIsObjs, bool a_bIsEnableAssert = true) {
		...
		if(a_oUIsObjs != null) {
			...
			var oPosList = new List<Vector3>() {
				Vector3.zero, Vector3.zero, new Vector3(KCDefine.B_VAL_0_FLT, CSceneManager.DownOffset, KCDefine.B_VAL_0_FLT), new Vector3(KCDefine.B_VAL_0_FLT, CSceneManager.DownOffset, KCDefine.B_VAL_0_FLT)
			};
	...
	private static void DoSetupQuality(EQualityLevel a_eQualityLevel, bool a_bIsEnableSetupRenderingPipeline, bool a_bIsEnableExpensiveChange = false) {
		...
		for(int i = 0; i < oQualityLevelList.Count; ++i) {
			...
			var oIsSetupQualityOptsList = new List<bool>() {
				...
				QualitySettings.shadowCascade2Split.ExIsEquals(KCDefine.B_VAL_1_FLT / KCDefine.B_VAL_4_FLT),
			...
			if(oIsSetupQualityOptsList.Contains(false)) {
				...
				QualitySettings.shadowCascade2Split = KCDefine.B_VAL_1_FLT / KCDefine.B_VAL_4_FLT;
