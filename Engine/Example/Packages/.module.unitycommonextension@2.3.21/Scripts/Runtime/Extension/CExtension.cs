...
public static partial class CExtension {
	...
	public static string ExToNumStr(this long a_nSender, bool a_bIsEnableFltPoint = false) {
		foreach(var stKeyVal in KCDefine.B_UNIT_NUM_TOKEN_INFO_DICT) {
			...
			// 변환 가능 할 경우
			if(nDigits > KCDefine.B_VAL_0_INT && nDigits < KCDefine.B_UNIT_DIGITS_PER_THOUSAND) {
				long nCorrectDigits01 = a_nSender / stKeyVal.Value;
				long nCorrectDigits02 = (a_nSender % stKeyVal.Value) / (stKeyVal.Value / KCDefine.B_UNIT_DIGITS_PER_TEN);
				
				return a_bIsEnableFltPoint ? string.Format(KCDefine.B_TEXT_FMT_NUM, nCorrectDigits01, nCorrectDigits02, stKeyVal.Key) : string.Format(KCDefine.B_TEXT_FMT_2_COMBINE, nCorrectDigits01, stKeyVal.Key);
			}
		}
	...
	public static Vector3 ExPosToPivotPos(this Vector3 a_stSender) {
		return new Vector3(a_stSender.x + (KCDefine.B_SCREEN_WIDTH / KCDefine.B_VAL_2_FLT), a_stSender.y + (KCDefine.B_SCREEN_HEIGHT / KCDefine.B_VAL_2_FLT), a_stSender.z);
	}
	...
	/** 값을 대체한다 */
	public static void ExReplaceVal<T>(this T[] a_oSender, T a_tVal, T a_tReplaceVal, System.Predicate<T> a_oCompare, bool a_bIsEnableAssert = true) {
		CAccess.Assert(!a_bIsEnableAssert || (a_oSender != null && a_oCompare != null));
	...
	/** 값을 대체한다 */
	public static void ExReplaceVal<T>(this List<T> a_oSender, T a_tVal, T a_tReplaceVal, System.Predicate<T> a_oCompare, bool a_bIsEnableAssert = true) {
		CAccess.Assert(!a_bIsEnableAssert || (a_oSender != null && a_oCompare != null));
	...
	/** 값을 대체한다 */
	public static void ExReplaceVals<T>(this List<T> a_oSender, List<(T, T)> a_oValList, System.Predicate<T> a_oCompare, bool a_bIsEnableAssert = true) {
		CAccess.Assert(!a_bIsEnableAssert || (a_oSender != null && a_oValList != null && a_oCompare != null));

		// 값 대체가 가능 할 경우
		if(a_oSender != null && a_oValList != null && a_oCompare != null) {
			for(int i = 0; i < a_oValList.Count; ++i) {
				a_oSender.ExReplaceVal(a_oValList[i].Item1, a_oValList[i].Item2, a_oCompare, a_bIsEnableAssert);
			}
		}
	}
	...
	/** 값을 대체한다 */
	public static void ExReplaceVal<K, V>(this Dictionary<K, V> a_oSender, K a_tKey, V a_tVal, System.Predicate<V> a_oCompare, bool a_bIsEnableAssert = true) {
		CAccess.Assert(!a_bIsEnableAssert || (a_oSender != null && a_oCompare != null));

		// 딕셔너리가 존재 할 경우
		if(a_oSender != null && a_oCompare != null) {
			// 값 대체가 가능 할 경우
			if(a_oSender.ExFindVal(a_oCompare).Item1) {
				a_oSender[a_tKey] = a_tVal;
			} else {
				a_oSender.Add(a_tKey, a_tVal);
			}
	...
	/** 값을 대체한다 */
	public static void ExReplaceVals<K, V>(this Dictionary<K, V> a_oSender, Dictionary<K, V> a_oValDict, System.Predicate<V> a_oCompare, bool a_bIsEnableAssert = true) {
		CAccess.Assert(!a_bIsEnableAssert || (a_oSender != null && a_oValDict != null && a_oCompare != null));

		// 값 대체가 가능 할 경우
		if(a_oSender != null && a_oValDict != null && a_oCompare != null) {
			foreach(var stKeyVal in a_oValDict) {
				a_oSender.ExReplaceVal(stKeyVal.Key, stKeyVal.Value, a_oCompare, a_bIsEnableAssert);
			}
		}
	}
		...
		for(int i = 0; i < a_oSender.GetLength(KCDefine.B_VAL_0_INT); ++i) {
			for(int j = 0; j < a_oSender.GetLength(KCDefine.B_VAL_0_INT); ++j) {
				...
				if(a_oCompare(a_oSender[i, j])) {
					return new Vector3Int(j, i, KCDefine.B_IDX_INVALID);
				}
			}
		}
	...
	public static void ExCopyToMultiArray<T>(this T[] a_oSender, T[,] a_oDestVals, bool a_bIsEnableAssert = true) {
		...
		if(a_oSender != null && a_oDestVals != null) {
			for(int i = 0; i < a_oSender.Length; ++i) {
				a_oDestVals.ExSetVal(new Vector3Int(i % a_oDestVals.GetLength(KCDefine.B_VAL_1_INT), i / a_oDestVals.GetLength(KCDefine.B_VAL_1_INT), KCDefine.B_VAL_0_INT), a_oSender[i], a_bIsEnableAssert);
			}
		}
	}
