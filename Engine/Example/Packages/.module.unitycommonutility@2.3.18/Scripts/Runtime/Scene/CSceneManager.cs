		...
		if(CSceneManager.IsExistsMainCamera) {
			...
			try {
				...
				for(int i = 0; i < oCanvasPosList.Count; ++i) {
					...
					Gizmos.DrawLine(oCanvasPosList[nIdxA], oCanvasPosList[nIdxB]);
				}

				for(int i = 0; i < oScreenPosList.Count; ++i) {
					int nIdxA = (i + KCDefine.B_VAL_0_INT) % oScreenPosList.Count;
					int nIdxB = (i + KCDefine.B_VAL_1_INT) % oScreenPosList.Count;

					Gizmos.color = Color.green;
					Gizmos.DrawLine(oScreenPosList[nIdxA], oScreenPosList[nIdxB]);
				}
			} finally {
