...
public abstract partial class CSceneManager : CComponent {
	...
	private Tween m_oTestUIsAni = null;
	...
	public override void Awake() {
		...
		try {
			...
			if(Application.isPlaying) {
				...
				this.TestUIs?.SetActive(this.IsActiveScene);
#else
	...
	private void OnTouchTestUIsOpenBtn() {
		CAccess.AssignVal(ref m_oTestUIsAni, m_oUIsDict[EKey.TEST_CONTENTS_UIS]?.transform.DOLocalMoveX(KCDefine.B_VAL_0_FLT, KCDefine.U_DURATION_ANI).SetAutoKill().SetUpdate(true));
	}

	/** 테스트 UI 닫기 버튼을 눌렀을 경우 */
	private void OnTouchTestUIsCloseBtn() {
		CAccess.AssignVal(ref m_oTestUIsAni, m_oUIsDict[EKey.TEST_CONTENTS_UIS]?.transform.DOLocalMoveX(-this.ScreenWidth, KCDefine.U_DURATION_ANI).SetAutoKill().SetUpdate(true));
	}
#endif			// #if DEBUG || DEVELOPMENT_BUILD || PLAY_TEST_ENABLE
	#endregion			// 조건부 함수
