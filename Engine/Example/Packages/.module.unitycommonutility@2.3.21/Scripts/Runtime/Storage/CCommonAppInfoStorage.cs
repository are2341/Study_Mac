...
public partial class CCommonAppInfo : CCommonBaseInfo {
	...
	private const string KEY_FIREBASE_MSG_TOKEN = "FirebaseMsgToken";
	...
	[JsonIgnore][IgnoreMember] public bool IsFirstPlay {
		get { return bool.Parse(m_oStrDict.GetValueOrDefault(KEY_IS_FIRST_PLAY, KCDefine.B_TEXT_FALSE)); }
		set { m_oStrDict.ExReplaceVal(KEY_IS_FIRST_PLAY, $"{value}"); }
	}
...
#if (UNITY_IOS || UNITY_ANDROID) && FIREBASE_CLOUD_MSG_ENABLE
	[JsonIgnore][IgnoreMember] public string FirebaseMsgToken {
		get { return m_oStrDict.GetValueOrDefault(KEY_FIREBASE_MSG_TOKEN, string.Empty); }
		set { m_oStrDict.ExReplaceVal(KEY_FIREBASE_MSG_TOKEN, value); }
	}
#endif			// #if (UNITY_IOS || UNITY_ANDROID) && FIREBASE_CLOUD_MSG_ENABLE
	#endregion			// 프로퍼티
...
public partial class CCommonAppInfoStorage : CSingleton<CCommonAppInfoStorage> {
	...
	public bool IsTestDevice(string a_oAdsID) {
		var oAdsIDList = CCollectionManager.Inst.SpawnList<string>();
		...
		try {
			return oAdsIDList.ExIsValidIdx(oAdsIDList.FindIndex((a_oTestAdsID) => a_oAdsID.Equals(a_oTestAdsID)));
		} finally {
			CCollectionManager.Inst.DespawnList(oAdsIDList);
		}
	}
