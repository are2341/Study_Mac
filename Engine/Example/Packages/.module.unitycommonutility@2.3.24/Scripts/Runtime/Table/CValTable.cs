...
public partial class CValTable : CSingleton<CValTable> {
	...
	private List<object> DoLoadVals(string a_oCSVStr) {
		...
		for(int i = 0; i < oValInfoList.Count; ++i) {
			...
			switch((EValType)int.Parse(oValInfoList[i][KCDefine.U_KEY_VAL_T_VAL_TYPE])) {
				case EValType.INT: this.AddInt(oKey, int.Parse(oVal), int.Parse(oValInfoList[i][KCDefine.U_KEY_REPLACE]) != KCDefine.B_VAL_0_INT); break;
				case EValType.REAL: this.AddReal(oKey, float.Parse(oVal), int.Parse(oValInfoList[i][KCDefine.U_KEY_REPLACE]) != KCDefine.B_VAL_0_INT); break;
				case EValType.STR: this.AddStr(oKey, oVal, int.Parse(oValInfoList[i][KCDefine.U_KEY_REPLACE]) != KCDefine.B_VAL_0_INT); break;
			}
		}
