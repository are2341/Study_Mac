...
public abstract partial class CPopup : CComponent {
	...
	private Dictionary<EKey, GameObject> m_oUIsDict = new Dictionary<EKey, GameObject>() {
		[EKey.CONTENTS] = null,
		[EKey.CONTENTS_UIS] = null,
		[EKey.TOUCH_RESPONDER] = null
	};
	#endregion			// 변수
	...
	protected virtual Image BGImg => m_oUIsDict[EKey.TOUCH_RESPONDER]?.GetComponentInChildren<Image>();

	protected GameObject Contents => m_oUIsDict[EKey.CONTENTS];
	protected GameObject ContentsUIs => m_oUIsDict[EKey.CONTENTS_UIS];
	protected GameObject TouchResponder => m_oUIsDict[EKey.TOUCH_RESPONDER];
	#endregion			// 프로퍼티
	...
	public override void Awake() {
		...
		m_oUIsDict[EKey.CONTENTS] = this.gameObject.ExFindChild(KCDefine.U_OBJ_N_CONTENTS);
		m_oUIsDict[EKey.CONTENTS_UIS] = this.gameObject.ExFindChild(KCDefine.U_OBJ_N_CONTENTS_UIS);

		// 터치 응답자를 설정한다
		m_oUIsDict[EKey.TOUCH_RESPONDER] = this.CreateTouchResponder();
		m_oUIsDict[EKey.TOUCH_RESPONDER]?.transform.SetAsFirstSibling();

		// 버튼을 설정한다
		m_oUIsDict[EKey.CONTENTS].ExFindComponent<Button>(KCDefine.U_OBJ_N_CLOSE_BTN)?.onClick.AddListener(this.OnTouchCloseBtn);
	}
	...
	public virtual void Init() {
		switch(this.AniType) {
			case EAniType.SCALE: {
				m_oUIsDict[EKey.CONTENTS].transform.localScale = new Vector3(KCDefine.U_MIN_SCALE_POPUP, KCDefine.U_MIN_SCALE_POPUP, KCDefine.U_MIN_SCALE_POPUP);
				m_oUIsDict[EKey.CONTENTS].transform.localPosition = Vector3.zero;
			} break;
			case EAniType.DROPDOWN: case EAniType.SLIDE_LEFT: case EAniType.SLIDE_RIGHT: {
				m_oUIsDict[EKey.CONTENTS].transform.localScale = Vector3.one;

				// 낙하 애니메이션 일 경우
				if(this.AniType == EAniType.DROPDOWN) {
					m_oUIsDict[EKey.CONTENTS].transform.localPosition = new Vector3(KCDefine.B_VAL_0_FLT, CSceneManager.CanvasSize.y, KCDefine.B_VAL_0_FLT);
				} else {
					m_oUIsDict[EKey.CONTENTS].transform.localPosition = new Vector3((this.AniType == EAniType.SLIDE_LEFT) ? CSceneManager.CanvasSize.x : -CSceneManager.CanvasSize.x, KCDefine.B_VAL_0_FLT, KCDefine.B_VAL_0_FLT);
				}
			} break;
		}
	...
	public void SetBGColor(Color a_stColor, bool a_bIsAni = true, float a_fDuration = KCDefine.U_DURATION_ANI) {
		m_oAniDict[EKey.BG_ANI]?.Kill();
		
		// 애니메이션 모드 일 경우
		if(a_bIsAni && !this.IsIgnoreAni) {
			m_oAniDict[EKey.BG_ANI] = this.BGImg.DOColor(a_stColor, a_fDuration).SetUpdate(true);
		} else {
	...
	public virtual void Show(System.Action<CPopup> a_oShowCallback, System.Action<CPopup> a_oCloseCallback) {
		// 객체가 존재 할 경우
		if(!this.IsDestroy && (!m_bIsShow && !m_bIsClose)) {
			m_bIsShow = true;
	...
	protected virtual void SetupContents() {
		...
		if(!this.IsIgnoreRebuildLayout) {
			// 컴포넌트를 설정한다
			m_oUIsDict[EKey.CONTENTS].ExEnumerateComponents<RectTransform>((a_nTrans) => { LayoutRebuilder.ForceRebuildLayoutImmediate(a_nTrans); return true; });
		}
	}
	...
	protected virtual Tween MakeShowAni() {
		switch(this.AniType) {
			case EAniType.SCALE: return m_oUIsDict[EKey.CONTENTS].transform.DOScale(KCDefine.U_SCALE_POPUP, KCDefine.U_DURATION_POPUP_SCALE_ANI).SetEase(Ease.OutBack);
			case EAniType.DROPDOWN: return m_oUIsDict[EKey.CONTENTS].transform.DOLocalMoveY(KCDefine.B_VAL_0_FLT, KCDefine.U_DURATION_POPUP_DROPDOWN_ANI).SetEase(Ease.OutBack);
			case EAniType.SLIDE_LEFT: case EAniType.SLIDE_RIGHT: return m_oUIsDict[EKey.CONTENTS].transform.DOLocalMoveX(KCDefine.B_VAL_0_FLT, KCDefine.U_DURATION_POPUP_SLIDE_ANI).SetEase(Ease.OutBack);
		}
	...
	protected virtual Tween MakeCloseAni() {
		switch(this.AniType) {
			case EAniType.SCALE: return m_oUIsDict[EKey.CONTENTS].transform.DOScale(KCDefine.U_MIN_SCALE_POPUP, KCDefine.U_DURATION_POPUP_SCALE_ANI).SetEase(Ease.InBack);
			case EAniType.DROPDOWN: return m_oUIsDict[EKey.CONTENTS].transform.DOLocalMoveY(CSceneManager.CanvasSize.y, KCDefine.U_DURATION_POPUP_DROPDOWN_ANI).SetEase(Ease.InBack);
			case EAniType.SLIDE_LEFT: case EAniType.SLIDE_RIGHT: return m_oUIsDict[EKey.CONTENTS].transform.DOLocalMoveX((this.AniType == EAniType.SLIDE_LEFT) ? -CSceneManager.CanvasSize.x : CSceneManager.CanvasSize.x, KCDefine.U_DURATION_POPUP_SLIDE_ANI).SetEase(Ease.InBack);
		}
	...
	private void StartShowAni() {
		...
		if(!this.IsIgnoreAni && this.AniType != EAniType.NONE) {
			m_oAniDict[EKey.SHOW_ANI] = CFactory.MakeSequence(this.MakeShowAni(), (a_oSender) => this.OnCompleteShowAni(), KCDefine.B_VAL_0_FLT, false, true);
		} else {
	...
	private void StartCloseAni() {
		...
		// 애니메이션 모드 일 경우
		if(!this.IsIgnoreAni && this.AniType != EAniType.NONE) {
			m_oAniDict[EKey.CLOSE_ANI] = CFactory.MakeSequence(this.MakeCloseAni(), (a_oSender) => this.OnCompleteCloseAni(), KCDefine.B_VAL_0_FLT, false, true);
		} else {
