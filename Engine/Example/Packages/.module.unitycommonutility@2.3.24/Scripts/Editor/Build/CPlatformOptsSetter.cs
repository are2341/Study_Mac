...
public static partial class CPlatformOptsSetter {
	...
	public static void EditorInitialize() {
		...
		if(CPlatformOptsSetter.DefineSymbolInfoTable != null) {
			...
			var oExtraDefineSymbolInfoList04 = new List<(string, string)>() {
			...
			foreach(var stKeyVal in CPlatformOptsSetter.DefineSymbolDictContainer) {
				...
				if(stKeyVal.Value.Contains(KCEditorDefine.DS_DEFINE_S_SAMPLE_PROJ)) {
					for(int i = 0; i < oExtraDefineSymbolInfoList01.Count; ++i) {
						// 추가 전처리기 심볼이 없을 경우
						if(!CEditorAccess.IsContainsDefineSymbol(stKeyVal.Key, oExtraDefineSymbolInfoList01[i].Item1) && !CEditorAccess.IsContainsDefineSymbol(stKeyVal.Key, oExtraDefineSymbolInfoList01[i].Item2)) {
							stKeyVal.Value.ExAddVal(oExtraDefineSymbolInfoList01[i].Item1);
						} else {
				...
				for(int i = 0; i < oExtraDefineSymbolInfoList02.Count; ++i) {
					// 추가 전처리기 심볼이 없을 경우
					if(!CEditorAccess.IsContainsDefineSymbol(stKeyVal.Key, oExtraDefineSymbolInfoList02[i].Item1) && !CEditorAccess.IsContainsDefineSymbol(stKeyVal.Key, oExtraDefineSymbolInfoList02[i].Item2)) {
				...
				for(int i = 0; i < oExtraDefineSymbolInfoList03.Count; ++i) {
					// 추가 전처리기 심볼이 존재 할 경우
					if(stKeyVal.Value.Contains(oExtraDefineSymbolInfoList03[i].Item1)) {
						stKeyVal.Value.ExAddVal(oExtraDefineSymbolInfoList03[i].Item2);
					}
				}

				for(int i = 0; i < oExtraDefineSymbolInfoList04.Count; ++i) {
					// 추가 전처리기 심볼이 없을 경우
					if(!stKeyVal.Value.Contains(oExtraDefineSymbolInfoList04[i].Item1)) {
						stKeyVal.Value.ExAddVal(oExtraDefineSymbolInfoList04[i].Item2);
					}
				}
				...
				stKeyVal.Value.ExAddVal(KCEditorDefine.DS_DEFINE_S_EDITOR_COROUTINE_ENABLE);

#if !SAMPLE_PROJ && !DEVELOPMENT_PROJ
				var oRemoveDefineSymbolList = new List<string>() {
					KCEditorDefine.DS_DEFINE_S_ML_AGENTS_ENABLE,
					KCEditorDefine.DS_DEFINE_S_ML_AGENTS_MODULE_ENABLE,

					KCEditorDefine.DS_DEFINE_S_CINEMACHINE_ENABLE,
					KCEditorDefine.DS_DEFINE_S_CINEMACHINE_MODULE_ENABLE,

					KCEditorDefine.DS_DEFINE_S_INPUT_SYSTEM_ENABLE,
					KCEditorDefine.DS_DEFINE_S_INPUT_SYSTEM_MODULE_ENABLE,

					KCEditorDefine.DS_DEFINE_S_POST_PROCESSING_ENABLE,
					KCEditorDefine.DS_DEFINE_S_POST_PROCESSING_MODULE_ENABLE,

					KCEditorDefine.DS_DEFINE_S_SKELETON_ANI_2D_ENABLE,
					KCEditorDefine.DS_DEFINE_S_SKELETON_ANI_2D_MODULE_ENABLE,

					KCEditorDefine.DS_DEFINE_S_UNIVERSAL_RENDERING_PIPELINE_ENABLE,
					KCEditorDefine.DS_DEFINE_S_UNIVERSAL_RENDERING_PIPELINE_MODULE_ENABLE
				};

				for(int i = 0; i < oRemoveDefineSymbolList.Count; ++i) {
					stKeyVal.Value.ExRemoveVal(oRemoveDefineSymbolList[i]);
				}
