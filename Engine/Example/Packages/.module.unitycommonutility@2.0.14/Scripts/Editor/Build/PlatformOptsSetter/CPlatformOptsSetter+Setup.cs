...
public static partial class CPlatformOptsSetter {
	...
	private static void SetupOpts() {
		...
		if(CPlatformOptsSetter.BuildInfoTable != null) {
			var oIsSetupStandaloneOptsList = new List<bool>() {
			...
			// 설정 갱신이 필요 할 경우
			if(oIsSetupStandaloneOptsList.Contains(false)) {
	...
	private static void SetupBuildOpts() {
		...
		if(CPlatformOptsSetter.ProjInfoTable != null) {
			var oIsSetupBuildOptsList = new List<bool>() {
				PlayerSettings.companyName.Equals(CPlatformOptsSetter.ProjInfoTable.Company),
				PlayerSettings.productName.Equals(CPlatformOptsSetter.ProjInfoTable.ProductName)
			};
	...
	private static void SetupiOSBuildOpts() {
		var oIsSetupOptsList = new List<bool>() {
			...
			(bool)CAccessExtension.ExGetPropertyVal<PlayerSettings.iOS>(null, KCEditorDefine.B_PROPERTY_N_APPLE_ENABLE_PRO_MOTION, KCDefine.B_BINDING_F_NON_PUBLIC_STATIC),
		...
		if(CPlatformOptsSetter.OptsInfoTable != null) {
			var oIsSetupBuildOptsList = new List<bool>() {
				// iOS
				PlayerSettings.iOS.cameraUsageDescription.Equals(CPlatformOptsSetter.OptsInfoTable.BuildOptsInfo.m_stiOSBuildOptsInfo.m_oCameraDescription),
				PlayerSettings.iOS.locationUsageDescription.Equals(CPlatformOptsSetter.OptsInfoTable.BuildOptsInfo.m_stiOSBuildOptsInfo.m_oLocationDescription),
				PlayerSettings.iOS.microphoneUsageDescription.Equals(CPlatformOptsSetter.OptsInfoTable.BuildOptsInfo.m_stiOSBuildOptsInfo.m_oMicrophoneDescription),
