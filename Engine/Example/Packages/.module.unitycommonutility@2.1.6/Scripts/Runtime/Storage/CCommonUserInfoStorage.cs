...
public class CCommonUserInfo : CCommonBaseInfo {
	...
	[JsonIgnore][IgnoreMember] public bool IsRemoveAds {
		get { return bool.Parse(m_oStrDict.GetValueOrDefault(CCommonUserInfo.KEY_IS_REMOVE_ADS, KCDefine.B_TEXT_FALSE)); }
		set { m_oStrDict.ExReplaceVal(CCommonUserInfo.KEY_IS_REMOVE_ADS, $"{value}"); }
	}

	[JsonIgnore][IgnoreMember] public EUserType UserType {
		get { return (EUserType)int.Parse(m_oStrDict.GetValueOrDefault(CCommonUserInfo.KEY_USER_TYPE, $"{(int)EUserType.NONE}")); }
		set { m_oStrDict.ExReplaceVal(CCommonUserInfo.KEY_USER_TYPE, $"{(int)value}"); }
	}
	#endregion			// 프로퍼티
