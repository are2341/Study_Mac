...
public abstract class CStudySceneManager : CSceneManager {
	...
	private void SetupAwake() {
		...
		m_oScrollViewContents = oScrollRect?.gameObject.ExFindChild(KCDefine.U_OBJ_N_CONTENTS);

		// 컨텐츠가 존재 할 경우
		if(m_oScrollViewContents != null) {
			for(int i = 0; i < KCDefine.B_UNIT_DIGITS_PER_TEN; ++i) {
				m_oEmptyTextList.Add(CFactory.CreateCloneObj<TMP_Text>(KCDefine.U_OBJ_N_EMPTY, KSDefine.SS_OBJ_P_BTN, m_oScrollViewContents));
			}
		}
		// 컨텐츠를 설정한다 }
	}
