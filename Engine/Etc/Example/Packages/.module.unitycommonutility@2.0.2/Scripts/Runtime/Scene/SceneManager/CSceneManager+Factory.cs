...
public abstract partial class CSceneManager : CComponent {
	...
	protected virtual Sequence CreateScreenFadeInAni(GameObject a_oTarget, string a_oKey, Color a_stColor, float a_fDuration) {
		...
		return CFactory.MakeSequence(oImg.DOColor(a_stColor, a_fDuration).SetEase(KCDefine.U_EASE_ANI), null, KCDefine.B_VAL_0_FLT, KCDefine.U_EASE_ANI, false, this.IsRealtimeFadeInAni);
#else
...
#if DOTWEEN_ENABLE
		var oImg = a_oTarget.GetComponentInChildren<Image>();
		return CFactory.MakeSequence(oImg.DOColor(a_stColor, a_fDuration).SetEase(KCDefine.U_EASE_ANI), null, KCDefine.B_VAL_0_FLT, KCDefine.U_EASE_ANI, false, this.IsRealtimeFadeOutAni);
#else
		return DOTween.Sequence();
#endif			// #if DOTWEEN_ENABLE
	}
