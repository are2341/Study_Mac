...
public abstract partial class CSceneManager : CComponent {
		...
		if(CSceneManager.IsExistsMainCamera) {
			...
			for(int i = 0; i < oCanvasPosList.Count; ++i) {
				...
				try {
					...
					Gizmos.DrawLine(oCanvasPosList[nIdxA] * (CAccess.ResolutionScale * KCDefine.B_UNIT_SCALE), oCanvasPosList[nIdxB] * (CAccess.ResolutionScale * KCDefine.B_UNIT_SCALE));

					Gizmos.color = Color.green;
					Gizmos.DrawLine(oScreenPosList[nIdxA] * (CAccess.ResolutionScale * KCDefine.B_UNIT_SCALE), oScreenPosList[nIdxB] * (CAccess.ResolutionScale * KCDefine.B_UNIT_SCALE));

#if UNITY_STANDALONE && MODE_PORTRAIT_ENABLE
					Gizmos.color = Color.cyan;
					Gizmos.DrawLine(oEditorScreenPosList[nIdxA] * KCDefine.B_UNIT_SCALE, oEditorScreenPosList[nIdxB] * KCDefine.B_UNIT_SCALE);
#endif			// #if UNITY_STANDALONE && MODE_PORTRAIT_ENABLE
				} finally {
	...
	public virtual void OnGUI() {
		...
		if(this.IsActiveScene && CSceneManager.ActiveSceneMainCamera != null) {
			...
			try {
				...
				CSceneManager.m_fFPSInfoSkipTime += CScheduleManager.Inst.UnscaleDeltaTime;
	...
	private void UpdateDebugUIsState(float a_fDeltaTime) {
		CSceneManager.m_fDebugInfoSkipTime += CScheduleManager.Inst.UnscaleDeltaTime;
		...
		if(CSceneManager.IsAppInit && bIsTimeScaleKeyDown) {
			...
			Time.timeScale = Mathf.Clamp01(Time.timeScale + (fVelocity * CScheduleManager.Inst.UnscaleDeltaTime));
		}
