...
public static class CScriptableObjCreator {
	...
	public static void CreateOptsInfoTable() {
		...
		oOptsInfoTable.SetBuildOptsInfo(new STBuildOptsInfo() {
			m_bIsPreBakeCollisionMesh = false, m_bIsPreserveFrameBufferAlpha = false,

			m_stiOSBuildOptsInfo = new STiOSBuildOptsInfo() {
				m_oCameraDescription = string.Empty, m_oMotionDescription = string.Empty, m_oLocationDescription = string.Empty, m_oMicrophoneDescription = string.Empty
			},
	...
	public static void CreateBuildInfoTable() {
		...
		oBuildInfoTable.SetJenkinsInfo(new STJenkinsInfo() {
			m_oUserID = "dante", m_oBranch = "master", m_oSrc = "02000111.Sample_Unity", m_oWorkspace = "/Users/dante/Documents/jenkins/workspace", m_oBuildToken = "JenkinsBuild", m_oAccessToken = "11736e2ed3cbca541394372e057ad0ba5a", m_oBuildURLFmt = "http://localhost:8080/{0}/buildWithParameters"
		});
		...
		oBuildInfoTable.SetiOSBuildInfo(new STiOSBuildInfo() {
			m_oTeamID = "8XBEE2A299", m_oTargetOSVer = "11.0", m_oDevProfileID = "c964937f-b8a3-4a09-b910-f74280cde2e7", m_oStoreProfileID = "5cc0de0a-785f-47c6-ab1e-b7274b35dca3"
		});
	...
	public static void CreateDefineSymbolInfoTable() {
		...
		oDefineSymbolInfoTable.SetSubCommonDefineSymbols(new List<string>() {
			...
			KCEditorDefine.DS_DEFINE_S_NINETAP_BUILD_PIPELINE_ENABLE,
			KCEditorDefine.DS_DEFINE_S_MODULE_VER_2_0_0_OR_NEWER,
			KCEditorDefine.DS_DEFINE_S_MODULE_VER_2_0_1_OR_NEWER,
			KCEditorDefine.DS_DEFINE_S_MODULE_VER_2_0_2_OR_NEWER
		});
	}
	...
	public static void CreateProjInfoTable() {
		...
		oProjInfoTable.SetProjName("02000111.Sample_Unity");
		...
		oProjInfoTable.SetAndroidOneStoreProjInfo(new STProjInfo() {
			m_stBuildVerInfo = new STBuildVerInfo() {
				m_nNum = 1, m_oVer = "0.0.1",
			},
		...
		oProjInfoTable.SetStandaloneMacAppleProjInfo(new STProjInfo() {
			m_stBuildVerInfo = new STBuildVerInfo() {
				m_nNum = 1, m_oVer = "0.0.1",
			},
	...
	public static void CreateLocalizeInfoTable() {
		...
		oLocalizeInfoTable.SetFontInfos(new List<STFontInfo>() {
			new STFontInfo() {
...
#if ADMOB_ADS_ENABLE
		/*
		iOS AppID: ca-app-pub-4429226069711533~9313515606
		Android AppID: ca-app-pub-4429226069711533~7000833607
		*/
		...
		oPluginInfoTable.SetAppsFlyerPluginInfo(new STAppsFlyerPluginInfo() {
			m_oAppID = string.Empty, m_oDevKey = string.Empty
		});
