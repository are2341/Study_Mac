...
public abstract partial class CSceneManager : CComponent {
	...
	protected virtual void SetupScene(bool a_bIsMainSetup) {
		...
		if(a_bIsMainSetup) {
			...
			m_oCanvasDict.GetValueOrDefault(EKey.POPUP_UIS_CANVAS).ExSetSortingOrder(new STSortingOrderInfo() {
			...
			m_oCanvasDict.GetValueOrDefault(EKey.TOPMOST_UIS_CANVAS).ExSetSortingOrder(new STSortingOrderInfo() {
...
#if MODE_2D_ENABLE
				var oRaycaster = m_oCameraDict.GetValueOrDefault(EKey.MAIN_CAMERA).gameObject.ExAddComponent<Physics2DRaycaster>();
#else
				var oRaycaster = m_oCameraDict.GetValueOrDefault(EKey.MAIN_CAMERA).gameObject.ExAddComponent<PhysicsRaycaster>();
	...
	protected virtual void SetupAdditiveScene() {
		...
		m_oObjDict.ExReplaceVal(EKey.OBJS_BASE, this.gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_OBJS_BASE));
	...
	protected virtual void SetupLights() {
		...
		if(this.IsActiveScene) {
			...
			if(oIsSetupOptsList.Contains(false)) {
				...
				RenderSettings.ambientIntensity = KCDefine.B_VAL_1_REAL;
				RenderSettings.reflectionIntensity = KCDefine.B_VAL_1_REAL;
		...
		this.gameObject.scene.ExEnumerateComponents<Light>((a_oLight) => {
			...
			a_oLight.intensity = a_oLight.name.Equals(KCDefine.U_OBJ_N_SCENE_MAIN_LIGHT) ? KCDefine.B_VAL_1_REAL : Mathf.Clamp(a_oLight.intensity, KCDefine.B_VAL_0_REAL, KCDefine.B_ADDITIONAL_LIGHT_INTENSITY);
	...
	protected virtual void SetupOffsets() {
		...
		if(CSceneManager.ActiveSceneUIsCanvas != null) {
			...
			CSceneManager.CanvasScale = new Vector3(CAccess.ResolutionScale, CAccess.ResolutionScale, CAccess.ResolutionScale);
		}
	...
	protected virtual void SetupRootObjs() {
		...
		m_oObjDict.GetValueOrDefault(EKey.BASE).transform.localPosition = new Vector3(KCDefine.B_VAL_0_REAL, KCDefine.B_VAL_0_REAL, -this.PlaneDistance);

		m_oObjDict.GetValueOrDefault(EKey.OBJS_BASE).ExReset();
		m_oObjDict.GetValueOrDefault(EKey.OBJS_BASE).transform.localScale = CSceneManager.CanvasScale * KCDefine.B_UNIT_SCALE;
		m_oObjDict.GetValueOrDefault(EKey.OBJS_BASE).transform.localPosition = Vector3.zero;

		m_oObjDict.GetValueOrDefault(EKey.UP_OBJS).ExReset();
		m_oObjDict.GetValueOrDefault(EKey.UP_OBJS).transform.localPosition = new Vector3(KCDefine.B_VAL_0_REAL, (CSceneManager.CanvasSize.y / KCDefine.B_VAL_2_REAL) + CSceneManager.UpOffset, KCDefine.B_VAL_0_REAL);

		m_oObjDict.GetValueOrDefault(EKey.DOWN_OBJS).ExReset();
		m_oObjDict.GetValueOrDefault(EKey.DOWN_OBJS).transform.localPosition = new Vector3(KCDefine.B_VAL_0_REAL, (CSceneManager.CanvasSize.y / -KCDefine.B_VAL_2_REAL) + CSceneManager.DownOffset, KCDefine.B_VAL_0_REAL);

		m_oObjDict.GetValueOrDefault(EKey.LEFT_OBJS).ExReset();
		m_oObjDict.GetValueOrDefault(EKey.LEFT_OBJS).transform.localPosition = new Vector3((CSceneManager.CanvasSize.x / -KCDefine.B_VAL_2_REAL) + CSceneManager.LeftOffset, KCDefine.B_VAL_0_REAL, KCDefine.B_VAL_0_REAL);

		m_oObjDict.GetValueOrDefault(EKey.RIGHT_OBJS).ExReset();
		m_oObjDict.GetValueOrDefault(EKey.RIGHT_OBJS).transform.localPosition = new Vector3((CSceneManager.CanvasSize.x / KCDefine.B_VAL_2_REAL) + CSceneManager.RightOffset, KCDefine.B_VAL_0_REAL, KCDefine.B_VAL_0_REAL);

		// 루트 객체 간격을 설정한다
		CSceneManager.UpRootOffset = (CSceneManager.ActiveSceneUpUIs.transform as RectTransform).anchoredPosition.y - ((CSceneManager.ActiveSceneUIs.transform as RectTransform).anchoredPosition.y + (this.ScreenHeight / KCDefine.B_VAL_2_REAL));
		CSceneManager.DownRootOffset = (CSceneManager.ActiveSceneDownUIs.transform as RectTransform).anchoredPosition.y - ((CSceneManager.ActiveSceneUIs.transform as RectTransform).anchoredPosition.y - (this.ScreenHeight / KCDefine.B_VAL_2_REAL));
		CSceneManager.LeftRootOffset = (CSceneManager.ActiveSceneLeftUIs.transform as RectTransform).anchoredPosition.x - ((CSceneManager.ActiveSceneUIs.transform as RectTransform).anchoredPosition.x - (this.ScreenWidth / KCDefine.B_VAL_2_REAL));
		CSceneManager.RightRootOffset = (CSceneManager.ActiveSceneRightUIs.transform as RectTransform).anchoredPosition.x - ((CSceneManager.ActiveSceneUIs.transform as RectTransform).anchoredPosition.x + (this.ScreenWidth / KCDefine.B_VAL_2_REAL));
	...
	protected virtual void SetupDefObjs() {
		...
		CSceneManager.m_oActiveSceneManagerDict.ExReplaceVal(EKey.SCENE_MANAGER, CSceneManager.ActiveScene.ExFindComponent<CSceneManager>(KCDefine.U_OBJ_N_SCENE_MANAGER));

		// UI 를 설정한다 {
		m_oUIsDict.ExReplaceVal(EKey.UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_UIS));
		m_oUIsDict.ExReplaceVal(EKey.UIS_ROOT, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_UIS_ROOT));
		m_oUIsDict.ExReplaceVal(EKey.UIS_BASE, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_UIS_BASE));

		m_oUIsDict.ExReplaceVal(EKey.PIVOT_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_PIVOT_UIS));
		m_oUIsDict.ExReplaceVal(EKey.ANCHOR_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_ANCHOR_UIS));
		m_oUIsDict.ExReplaceVal(EKey.CORNER_ANCHOR_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_CORNER_ANCHOR_UIS));

		m_oUIsDict.ExReplaceVal(EKey.UP_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_UP_UIS));
		m_oUIsDict.ExReplaceVal(EKey.DOWN_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_DOWN_UIS));
		m_oUIsDict.ExReplaceVal(EKey.LEFT_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_LEFT_UIS));
		m_oUIsDict.ExReplaceVal(EKey.RIGHT_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_RIGHT_UIS));
		
		m_oUIsDict.ExReplaceVal(EKey.UP_LEFT_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_UP_LEFT_UIS));
		m_oUIsDict.ExReplaceVal(EKey.UP_RIGHT_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_UP_RIGHT_UIS));
		m_oUIsDict.ExReplaceVal(EKey.DOWN_LEFT_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_DOWN_LEFT_UIS));
		m_oUIsDict.ExReplaceVal(EKey.DOWN_RIGHT_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_DOWN_RIGHT_UIS));
		
		m_oUIsDict.ExReplaceVal(EKey.POPUP_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_POPUP_UIS));
		m_oUIsDict.ExReplaceVal(EKey.TOPMOST_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_TOPMOST_UIS));
		...
		CSceneManager.m_oActiveSceneUIsDict.ExReplaceVal(EKey.UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_UIS));
		CSceneManager.m_oActiveSceneUIsDict.ExReplaceVal(EKey.UIS_ROOT, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_UIS_ROOT));
		CSceneManager.m_oActiveSceneUIsDict.ExReplaceVal(EKey.UIS_BASE, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_UIS_BASE));

		CSceneManager.m_oActiveSceneUIsDict.ExReplaceVal(EKey.PIVOT_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_PIVOT_UIS));
		CSceneManager.m_oActiveSceneUIsDict.ExReplaceVal(EKey.ANCHOR_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_ANCHOR_UIS));
		CSceneManager.m_oActiveSceneUIsDict.ExReplaceVal(EKey.CORNER_ANCHOR_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_CORNER_ANCHOR_UIS));

		CSceneManager.m_oActiveSceneUIsDict.ExReplaceVal(EKey.UP_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_UP_UIS));
		CSceneManager.m_oActiveSceneUIsDict.ExReplaceVal(EKey.DOWN_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_DOWN_UIS));
		CSceneManager.m_oActiveSceneUIsDict.ExReplaceVal(EKey.LEFT_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_LEFT_UIS));
		CSceneManager.m_oActiveSceneUIsDict.ExReplaceVal(EKey.RIGHT_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_RIGHT_UIS));

		CSceneManager.m_oActiveSceneUIsDict.ExReplaceVal(EKey.UP_LEFT_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_UP_LEFT_UIS));
		CSceneManager.m_oActiveSceneUIsDict.ExReplaceVal(EKey.UP_RIGHT_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_UP_RIGHT_UIS));
		CSceneManager.m_oActiveSceneUIsDict.ExReplaceVal(EKey.DOWN_LEFT_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_DOWN_LEFT_UIS));
		CSceneManager.m_oActiveSceneUIsDict.ExRemoveVal(EKey.DOWN_RIGHT_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_DOWN_RIGHT_UIS));

		CSceneManager.m_oActiveSceneUIsDict.ExReplaceVal(EKey.POPUP_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_POPUP_UIS));
		CSceneManager.m_oActiveSceneUIsDict.ExReplaceVal(EKey.TOPMOST_UIS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_TOPMOST_UIS));
		// UI 를 설정한다 }

		// 객체를 설정한다 {
		m_oObjDict.ExReplaceVal(EKey.BASE, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_BASE));
		m_oObjDict.ExReplaceVal(EKey.OBJS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_OBJS));
		m_oObjDict.ExReplaceVal(EKey.OBJS_BASE, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_OBJS_BASE));
		
		m_oObjDict.ExReplaceVal(EKey.PIVOT_OBJS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_PIVOT_OBJS));
		m_oObjDict.ExReplaceVal(EKey.ANCHOR_OBJS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_ANCHOR_OBJS));
		m_oObjDict.ExReplaceVal(EKey.STATIC_OBJS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_STATIC_OBJS));

		m_oObjDict.ExReplaceVal(EKey.ADDITIONAL_LIGHTS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_ADDITIONAL_LIGHTS));
		m_oObjDict.ExReplaceVal(EKey.REFLECTION_PROBES, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_REFLECTION_PROBES));
		m_oObjDict.ExReplaceVal(EKey.LIGHT_PROBE_GROUPS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_LIGHT_PROBE_GROUPS));

		m_oObjDict.ExReplaceVal(EKey.UP_OBJS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_UP_OBJS));
		m_oObjDict.ExReplaceVal(EKey.DOWN_OBJS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_DOWN_OBJS));
		m_oObjDict.ExReplaceVal(EKey.LEFT_OBJS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_LEFT_OBJS));
		m_oObjDict.ExReplaceVal(EKey.RIGHT_OBJS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_RIGHT_OBJS));

		CSceneManager.m_oActiveSceneObjDict.ExReplaceVal(EKey.BASE, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_BASE));
		CSceneManager.m_oActiveSceneObjDict.ExReplaceVal(EKey.OBJS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_OBJS));
		CSceneManager.m_oActiveSceneObjDict.ExReplaceVal(EKey.OBJS_BASE, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_OBJS_BASE));

		CSceneManager.m_oActiveSceneObjDict.ExReplaceVal(EKey.PIVOT_OBJS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_PIVOT_OBJS));
		CSceneManager.m_oActiveSceneObjDict.ExReplaceVal(EKey.ANCHOR_OBJS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_ANCHOR_OBJS));
		CSceneManager.m_oActiveSceneObjDict.ExReplaceVal(EKey.STATIC_OBJS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_STATIC_OBJS));

		CSceneManager.m_oActiveSceneObjDict.ExReplaceVal(EKey.ADDITIONAL_LIGHTS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_ADDITIONAL_LIGHTS));
		CSceneManager.m_oActiveSceneObjDict.ExReplaceVal(EKey.REFLECTION_PROBES, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_REFLECTION_PROBES));
		CSceneManager.m_oActiveSceneObjDict.ExReplaceVal(EKey.LIGHT_PROBE_GROUPS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_LIGHT_PROBE_GROUPS));

		CSceneManager.m_oActiveSceneObjDict.ExReplaceVal(EKey.UP_OBJS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_UP_OBJS));
		CSceneManager.m_oActiveSceneObjDict.ExReplaceVal(EKey.DOWN_OBJS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_DOWN_OBJS));
		CSceneManager.m_oActiveSceneObjDict.ExReplaceVal(EKey.LEFT_OBJS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_LEFT_OBJS));
		CSceneManager.m_oActiveSceneObjDict.ExReplaceVal(EKey.RIGHT_OBJS, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindChild(KCDefine.U_OBJ_N_SCENE_RIGHT_OBJS));
		// 객체를 설정한다 }

		// 카메라를 설정한다
		m_oCameraDict.ExReplaceVal(EKey.MAIN_CAMERA, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindComponent<Camera>(KCDefine.U_OBJ_N_SCENE_MAIN_CAMERA));
		CSceneManager.m_oActiveSceneCameraDict.ExReplaceVal(EKey.MAIN_CAMERA, CSceneManager.m_oActiveSceneManagerDict.GetValueOrDefault(EKey.SCENE_MANAGER).gameObject.scene.ExFindComponent<Camera>(KCDefine.U_OBJ_N_SCENE_MAIN_CAMERA));
	...
	protected virtual void SetupMainCamera(Camera a_oCamera, bool a_bIsEnableAssert = true) {
		...
		if(a_oCamera != null) {
			// 트랜스 폼을 설정한다
			a_oCamera.transform.localScale = CSceneManager.CanvasScale * KCDefine.B_UNIT_SCALE;
	...
	private void SetupBlindUIs(GameObject a_oUIsObjs, bool a_bIsEnableAssert = true) {
		...
		if(a_oUIsObjs != null) {
			...
			var oPosList = new List<Vector3>() {
				Vector3.zero, Vector3.zero, new Vector3(KCDefine.B_VAL_0_REAL, CSceneManager.DownOffset, KCDefine.B_VAL_0_REAL), new Vector3(KCDefine.B_VAL_0_REAL, CSceneManager.DownOffset, KCDefine.B_VAL_0_REAL)
			};
			...
			float fDownOffset = Mathf.Clamp(oOffsetList[KCDefine.B_VAL_1_INT].y - CSceneManager.DownOffset, KCDefine.B_VAL_0_REAL, float.MaxValue);
			oOffsetList[KCDefine.B_VAL_1_INT] = new Vector3(oOffsetList[KCDefine.B_VAL_1_INT].x, fDownOffset, oOffsetList[KCDefine.B_VAL_1_INT].z);
	...
	private void SetupAnchorUIs(GameObject a_oUIsObjs, ref Vector3 a_rstPos, ref Vector3 a_rstSize, ref Vector3 a_rstPivot, bool a_bIsEnableAssert = true) {
		...
		if(a_oUIsObjs != null) {
			...
			if(bIsUpUIs || bIsDownUIs) {
				...
				// 위쪽 UI 일 경우
				if(bIsUpUIs) {
					a_rstPos = new Vector3(KCDefine.B_VAL_0_REAL, (CSceneManager.CanvasSize.y / KCDefine.B_VAL_2_REAL) + CSceneManager.UpOffset, KCDefine.B_VAL_0_REAL);
				} else {
					a_rstPos = new Vector3(KCDefine.B_VAL_0_REAL, (CSceneManager.CanvasSize.y / -KCDefine.B_VAL_2_REAL) + CSceneManager.DownOffset, KCDefine.B_VAL_0_REAL);
				}
			}
			// 왼쪽, 오른쪽 UI 일 경우
				...
				// 왼쪽 UI 일 경우
				if(bIsLeftUIs) {
					a_rstPos = new Vector3((CSceneManager.CanvasSize.x / -KCDefine.B_VAL_2_REAL) + CSceneManager.LeftOffset, KCDefine.B_VAL_0_REAL, KCDefine.B_VAL_0_REAL);
				} else {
					a_rstPos = new Vector3((CSceneManager.CanvasSize.x / KCDefine.B_VAL_2_REAL) + CSceneManager.RightOffset, KCDefine.B_VAL_0_REAL, KCDefine.B_VAL_0_REAL);
				}
			}
		}
	}
	...
	private void SetupCornerAnchorUIs(GameObject a_oUIsObjs, ref Vector3 a_rstPos, ref Vector3 a_rstSize, ref Vector3 a_rstPivot, ref Vector3 a_rstAnchorMin, ref Vector3 a_rstAnchorMax, bool a_bIsEnableAssert = true) {
		...
		if(a_oUIsObjs != null) {
			...
			if(bIsUpLeftUIs || bIsUpRightUIs) {
				...
				// 좌상단 UI 일 경우
				if(bIsUpLeftUIs) {
					a_rstPos = new Vector3(CSceneManager.LeftOffset, CSceneManager.UpOffset, KCDefine.B_VAL_0_REAL);
				} else {
					a_rstPos = new Vector3(CSceneManager.RightOffset, CSceneManager.UpOffset, KCDefine.B_VAL_0_REAL);
				}
			}
			// 좌하단, 우하단 UI 일 경우
				...
				// 좌하단 UI 일 경우
				if(bIsDownLeftUIs) {
					a_rstPos = new Vector3(CSceneManager.LeftOffset, CSceneManager.DownOffset, KCDefine.B_VAL_0_REAL);
				} else {
					a_rstPos = new Vector3(CSceneManager.RightOffset, CSceneManager.DownOffset, KCDefine.B_VAL_0_REAL);
				}
			}
		}
	}
	...
	private static void DoSetupQuality(EQualityLevel a_eQualityLevel, bool a_bIsEnableSetupRenderingPipeline, bool a_bIsEnableExpensiveChange = false) {
		...
		for(int i = 0; i < oQualityLevelList.Count; ++i) {
			...
			var oIsSetupQualityOptsList = new List<bool>() {
				...
				QualitySettings.shadowCascade2Split.ExIsEquals(KCDefine.B_VAL_1_REAL / KCDefine.B_VAL_4_REAL),
				...
				QualitySettings.antiAliasing == (int)EMSAAQuality._4x,
			...
			if(oIsSetupQualityOptsList.Contains(false)) {
				...
				QualitySettings.shadowCascade2Split = KCDefine.B_VAL_1_REAL / KCDefine.B_VAL_4_REAL;
	...
	private void SetupDebugUIs(GameObject a_oUIsObjs, bool a_bIsEnableAssert = true) {
		...
		if(a_oUIsObjs != null && CSceneManager.ScreenStaticDebugText != null) {
			CSceneManager.m_oStrBuilderDict.GetValueOrDefault(EKey.STATIC_DEBUG_STR_BUILDER).Clear();
			CSceneManager.m_oStrBuilderDict.GetValueOrDefault(EKey.STATIC_DEBUG_STR_BUILDER).AppendFormat(KCDefine.U_TEXT_FMT_STATIC_DEBUG_INFO_01, CSceneManager.UpOffset, CSceneManager.DownOffset, CSceneManager.LeftOffset, CSceneManager.RightOffset);
			CSceneManager.m_oStrBuilderDict.GetValueOrDefault(EKey.STATIC_DEBUG_STR_BUILDER).AppendFormat(KCDefine.U_TEXT_FMT_STATIC_DEBUG_INFO_02, CSceneManager.UpRootOffset, CSceneManager.DownRootOffset, CSceneManager.LeftRootOffset, CSceneManager.RightRootOffset);
			CSceneManager.m_oStrBuilderDict.GetValueOrDefault(EKey.STATIC_DEBUG_STR_BUILDER).AppendFormat(KCDefine.U_TEXT_FMT_STATIC_DEBUG_INFO_03, CAccess.ScreenSize.x, CAccess.ScreenSize.y);

#if ADS_MODULE_ENABLE
			CSceneManager.m_oStrBuilderDict.GetValueOrDefault(EKey.STATIC_DEBUG_STR_BUILDER).AppendFormat(KCDefine.U_TEXT_FMT_STATIC_DEBUG_INFO_04, CAccess.ScreenDPI, CAccess.GetBannerAdsHeight(KCDefine.U_SIZE_BANNER_ADS.y));
#endif			// #if ADS_MODULE_ENABLE

			CSceneManager.m_oStrBuilderDict.GetValueOrDefault(EKey.STATIC_DEBUG_STR_BUILDER).AppendFormat(KCDefine.U_TEXT_FMT_STATIC_DEBUG_INFO_05, (EQualityLevel)QualitySettings.GetQualityLevel(), Application.targetFrameRate);
			CSceneManager.m_oStrBuilderDict.GetValueOrDefault(EKey.STATIC_DEBUG_STR_BUILDER).AppendFormat(KCDefine.U_TEXT_FMT_STATIC_DEBUG_INFO_06, KCDefine.B_DIR_P_WRITABLE);
		}
	}
#endif			// #if DEBUG || DEVELOPMENT_BUILD
