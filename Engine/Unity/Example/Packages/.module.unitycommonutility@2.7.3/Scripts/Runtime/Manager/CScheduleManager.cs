...
public partial class CScheduleManager : CSingleton<CScheduleManager> {
	...
	#region 프로퍼티
	public float DeltaTime => m_oRealDict.GetValueOrDefault(EKey.DELTA_TIME);
	public float UnscaleDeltaTime => m_oRealDict.GetValueOrDefault(EKey.UNSCALE_DELTA_TIME);
	#endregion			// 프로퍼티
	...
	public virtual void Update() {
		...
		m_oRealDict.ExReplaceVal(EKey.DELTA_TIME, Mathf.Clamp(Time.deltaTime, KCDefine.B_VAL_0_REAL, fMaxDeltaTime));
		m_oRealDict.ExReplaceVal(EKey.UNSCALE_DELTA_TIME, Mathf.Clamp(Time.unscaledDeltaTime, KCDefine.B_VAL_0_REAL, fMaxDeltaTime));
		...
		for(int i = 0; i < m_oComponentInfoListWrapper.m_oList.Count; ++i) {
			...
			if(oComponent != null && (oComponent.enabled && !oComponent.IsDestroy && oComponent.gameObject.activeInHierarchy)) {
				oComponent.OnUpdate(m_oRealDict.GetValueOrDefault(EKey.DELTA_TIME));
			}
		}
	}
	...
	public virtual void LateUpdate() {
		float fUpdateSkipTime = m_oRealDict.GetValueOrDefault(EKey.UPDATE_SKIP_TIME);
		m_oRealDict.ExReplaceVal(EKey.UPDATE_SKIP_TIME, fUpdateSkipTime + m_oRealDict.GetValueOrDefault(EKey.UNSCALE_DELTA_TIME));
		...
		for(int i = 0; i < m_oComponentInfoListWrapper.m_oList.Count; ++i) {
			...
			if(oComponent == null || oComponent.IsDestroy) {
				...
				m_oComponentInfoListWrapper.m_oList.ExRemoveValAt(i + KCDefine.B_VAL_1_INT);
			}
			// 상태 갱신이 가능 할 경우
			else if(oComponent.enabled && oComponent.gameObject.activeInHierarchy) {
				oComponent.OnLateUpdate(m_oRealDict.GetValueOrDefault(EKey.DELTA_TIME));
			}
		}

		// 콜백 호출 주기가 지났을 경우
		if(m_oRealDict.GetValueOrDefault(EKey.UPDATE_SKIP_TIME).ExIsGreateEquals(KCDefine.U_DELTA_T_SCHEDULE_M_CALLBACK)) {
			m_oRealDict.ExReplaceVal(EKey.UPDATE_SKIP_TIME, KCDefine.B_VAL_0_REAL);
