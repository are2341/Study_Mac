...
public partial class CResManager : CSingleton<CResManager> {
	...
	private void SetupDefResources() {
		// 메쉬를 설정한다 {
		var oMesh = new Mesh();
		oMesh.name = KCDefine.U_MESH_N_DEF;
		...
		this.AddRes<Mesh>(KCDefine.U_MESH_N_DEF, oMesh);
		// 메쉬를 설정한다 }

		// 스프라이트를 설정한다 {
		var oSprite = Sprite.Create(Texture2D.whiteTexture, Rect.MinMaxRect(KCDefine.B_VAL_0_REAL, KCDefine.B_VAL_0_REAL, KCDefine.B_VAL_1_REAL, KCDefine.B_VAL_1_REAL), KCDefine.B_ANCHOR_MID_CENTER, KCDefine.B_VAL_1_REAL);
	...
	public T GetRes<T>(string a_oKey, bool a_bIsAutoCreate = true) where T : Object {
		...
		if(a_bIsAutoCreate && !oResDict.ContainsKey(a_oKey)) {
			oResDict.TryAdd(a_oKey, m_oResCreatorDict[typeof(T)](a_oKey) as T);
		}
