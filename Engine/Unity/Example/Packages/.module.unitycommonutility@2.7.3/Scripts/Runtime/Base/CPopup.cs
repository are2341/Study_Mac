...
public abstract partial class CPopup : CComponent {
	...
	protected virtual Tween MakeShowAni() {
		switch(this.AniType) {
			...
			case EAniType.SLIDE_LEFT: case EAniType.SLIDE_RIGHT: return m_oUIsDict.GetValueOrDefault(EKey.CONTENTS).transform.DOLocalMoveX(KCDefine.B_VAL_0_REAL, KCDefine.U_DURATION_POPUP_ANI).SetEase(Ease.OutBack);
		}
	...
	protected virtual Tween MakeCloseAni() {
		switch(this.AniType) {
			...
			case EAniType.SLIDE_LEFT: case EAniType.SLIDE_RIGHT: return m_oUIsDict.GetValueOrDefault(EKey.CONTENTS).transform.DOLocalMoveX((this.AniType == EAniType.SLIDE_LEFT) ? -CSceneManager.CanvasSize.x : CSceneManager.CanvasSize.x, KCDefine.U_DURATION_POPUP_ANI).SetEase(Ease.InBack);
		}
