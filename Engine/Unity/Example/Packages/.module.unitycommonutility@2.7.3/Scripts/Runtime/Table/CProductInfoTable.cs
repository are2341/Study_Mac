...
/** 상품 정보 */
[System.Serializable]
public partial struct STProductInfo {
...
public partial class CProductInfoTable : CScriptableObj<CProductInfoTable> {
...
#else
		try {
			return this.DoLoadProductInfos(CResManager.Inst.GetRes<TextAsset>(a_oFilePath).text);
		} finally {
	...
	private List<STProductInfo> DoLoadProductInfos(string a_oJSONStr) {
		...
		for(int i = 0; i < KCDefine.G_KEY_PRODUCT_INFOS.Count; ++i) {
			oProductInfosList.ExAddVal(oJSONNode[KCDefine.G_KEY_PRODUCT_INFOS[i]]);
		}
