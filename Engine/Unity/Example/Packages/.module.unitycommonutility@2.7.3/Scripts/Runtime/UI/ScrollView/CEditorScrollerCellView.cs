...
public partial class CEditorScrollerCellView : CScrollerCellView {
	...
	private void OnTouchCopyBtn() {
		this.Params.m_oCallbackDict?.GetValueOrDefault(ECallback.COPY)?.Invoke(this, this.Params.m_stBaseParams.m_nID);
	}

	/** 이동 버튼을 눌렀을 경우 */
	private void OnTouchMoveBtn() {
		this.Params.m_oCallbackDict?.GetValueOrDefault(ECallback.MOVE)?.Invoke(this, this.Params.m_stBaseParams.m_nID);
	}

	/** 제거 버튼을 눌렀을 경우 */
	private void OnTouchRemoveBtn() {
		this.Params.m_oCallbackDict?.GetValueOrDefault(ECallback.REMOVE)?.Invoke(this, this.Params.m_stBaseParams.m_nID);
	}
	#endregion			// 함수
}
