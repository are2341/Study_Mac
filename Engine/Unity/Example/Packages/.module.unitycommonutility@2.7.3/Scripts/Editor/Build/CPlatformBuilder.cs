...
public static partial class CPlatformBuilder {
	/** 젠킨스 매개 변수 */
	private partial struct STJenkinsParams {
	...
	private static void DoBuildPlatform(BuildPlayerOptions a_oPlayerOpts, List<string> a_oOutRestoreDefineSymbolList) {
		...
		// 빌드 옵션을 설정한다 {
		a_oPlayerOpts.options &= ~(BuildOptions.SymlinkSources | BuildOptions.CompressWithLz4 | BuildOptions.CompressWithLz4HC);
...
#else
			bool bIsEditorScene01 = EditorBuildSettings.scenes[i].path.Contains(KCDefine.B_EDITOR_SCENE_N_PATTERN_01);
			bool bIsEditorScene02 = EditorBuildSettings.scenes[i].path.Contains(KCDefine.B_EDITOR_SCENE_N_PATTERN_02);
