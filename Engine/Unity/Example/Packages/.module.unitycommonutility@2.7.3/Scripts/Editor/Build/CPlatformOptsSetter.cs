		...
		if(CPlatformOptsSetter.DefineSymbolInfoTable != null) {
			...
			foreach(var stKeyVal in CPlatformOptsSetter.DefineSymbolDictContainer) {
				...
				for(int i = 0; i < oExtraDefineSymbolInfoList01.Count; ++i) {
					// 추가 전처리기 심볼이 존재 할 경우
					if(stKeyVal.Value.Contains(oExtraDefineSymbolInfoList01[i].Item1)) {
				...
				if(stKeyVal.Value.Contains(KCEditorDefine.DS_DEFINE_S_SAMPLE_PROJ)) {
					...
					for(int i = 0; i < oExtraDefineSymbolInfoList.Count; ++i) {
						// 추가 전처리기 심볼이 존재 할 경우
						if(CEditorAccess.IsContainsDefineSymbol(stKeyVal.Key, oExtraDefineSymbolInfoList[i].Item1)) {
