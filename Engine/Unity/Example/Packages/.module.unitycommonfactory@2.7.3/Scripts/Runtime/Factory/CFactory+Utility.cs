...
public static partial class CFactory {
	...
	public static ulong MakeULevelID(int a_nLevelID, int a_nStageID = KCDefine.B_VAL_0_INT, int a_nChapterID = KCDefine.B_VAL_0_INT) {
		return CFactory.MakeUniqueStageID(a_nStageID, a_nChapterID) + (ulong)a_nLevelID;
	}

	/** 고유 스테이지 식별자를 생성한다 */
	public static ulong MakeUniqueStageID(int a_nStageID, int a_nChapterID = KCDefine.B_VAL_0_INT) {
		return CFactory.MakeUniqueChapterID(a_nChapterID) + ((ulong)a_nStageID * KCDefine.B_UNIT_IDS_PER_IDS_02);
	}

	/** 고유 챕터 식별자를 생성한다 */
	public static ulong MakeUniqueChapterID(int a_nChapterID) {
		return (ulong)a_nChapterID * KCDefine.B_UNIT_IDS_PER_IDS_03;
	}
	
	/** 식별자 정보를 생성한다 */
	public static STIDInfo MakeIDInfo(int a_nID01, int a_nID02 = KCDefine.B_VAL_0_INT, int a_nID03 = KCDefine.B_VAL_0_INT) {
		return new STIDInfo() {
			m_nID01 = a_nID01, m_nID02 = a_nID02, m_nID03 = a_nID03
		};
	}
