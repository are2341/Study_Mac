...
public class CStrTable : CSingleton<CStrTable> {
	...
	//! 문자열을 로드한다
	public Dictionary<string, string> LoadStrs(string a_oCSVStr) {
	...
	//! 문자열을 로드한다
	public Dictionary<string, string> LoadStrsFromFile(string a_oFilePath) {
		CAccess.Assert(a_oFilePath.ExIsValid());
		string oCSVStr = CFunc.ReadStr(a_oFilePath);

		return this.LoadStrs(oCSVStr);
	}
	...
	public Dictionary<string, string> LoadStrsFromRes(string a_oFilePath) {
		...
		try {
			...
			return this.LoadStrs(oTextAsset.text);
		} finally {
