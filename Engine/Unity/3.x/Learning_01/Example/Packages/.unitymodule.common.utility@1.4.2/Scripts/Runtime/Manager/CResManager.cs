...
public class CResManager : CSingleton<CResManager> {
	...
	public void LoadSpriteAtlas(string a_oFilePath, bool a_bIsAutoLoadSprites = true) {
		...
		if(a_bIsAutoLoadSprites) {
			...
			for(int i = 0; i < oSprites.Length; ++i) {
				...
				string oKey = oSprites[i].name.ExGetReplaceStr(oTarget, string.Empty);
	...
	private void SetupDefReses() {
		...
		var oSprite = Sprite.Create(Texture2D.whiteTexture, stRect, KCDefine.B_ANCHOR_MIDDLE_CENTER, KCDefine.B_VAL_1_FLT);
		
		oSprite.name = KCDefine.U_IMG_N_SPRITE;
		this.AddRes<Sprite>(KCDefine.U_IMG_N_SPRITE, oSprite);
		// 스프라이트를 설정한다 }
	}
	#endregion			// 함수
	...
	public void AddRes<T>(string a_oKey, T a_tRes, bool a_bIsReplace = false) where T : Object {
		...
		// 대체 모드 일 경우
		if(a_bIsReplace) {
			oResList.ExReplaceVal(a_oKey, a_tRes);
		} else {
			oResList.ExAddVal(a_oKey, a_tRes);
		}
	}
	...
	public void AddResCreator<T>(System.Func<string, Object> a_oCreator, bool a_bIsReplace = false) where T : Object {
		...
		// 대체 모드 일 경우
		if(a_bIsReplace) {
			m_oResCreatorList.ExReplaceVal(typeof(T), a_oCreator);
		} else {
			m_oResCreatorList.ExAddVal(typeof(T), a_oCreator);
		}
	}
	...
	public void RemoveResCreator<T>() where T : Object {
		m_oResCreatorList.ExRemoveVal(typeof(T));
	}
