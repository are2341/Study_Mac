...
public static partial class CPlatformBuilder {
	#region 클래스 함수
	//! 젠킨스 빌드를 실행한다
	public static void ExecuteJenkinsBuild(EBuildType a_eBuildType, string a_oPipeline, string a_oProjName, string a_oPlatform, string a_oBuildMode, string a_oBuildVer, string a_oBuildFunc, string a_oPipelineName, Dictionary<string, string> a_oDataList = null) {
		...
		oDataList.ExAddVal(KCEditorDefine.B_KEY_JENKINS_ENGINE_VER, Application.unityVersion);
		oDataList.ExAddVal(KCEditorDefine.B_KEY_JENKINS_BRANCH, oBranch);
		oDataList.ExAddVal(KCEditorDefine.B_KEY_JENKINS_SRC, oSrc);
		oDataList.ExAddVal(KCEditorDefine.B_KEY_JENKINS_PROJ_NAME, CPlatformOptsSetter.ProjInfoTable.ProjName);
		oDataList.ExAddVal(KCEditorDefine.B_KEY_JENKINS_PROJ_PATH, oProjPath);
		oDataList.ExAddVal(KCEditorDefine.B_KEY_JENKINS_PLATFORM, a_oPlatform);
		oDataList.ExAddVal(KCEditorDefine.B_KEY_JENKINS_ANALYTICS, oAnalytics);
		oDataList.ExAddVal(KCEditorDefine.B_KEY_JENKINS_BUILD_MODE, a_oBuildMode);
		oDataList.ExAddVal(KCEditorDefine.B_KEY_JENKINS_BUILD_VER, a_oBuildVer);
		oDataList.ExAddVal(KCEditorDefine.B_KEY_JENKINS_BUILD_FUNC, a_oBuildFunc);
		oDataList.ExAddVal(KCEditorDefine.B_KEY_JENKINS_PIPELINE_NAME, a_oPipelineName);
		
		foreach(var stKeyVal in oDataList) {
	...
	public static void ExecuteiOSJenkinsBuild(EBuildType a_eBuildType, string a_oBuildMode, string a_oBuildFunc, string a_oPipelineName, string a_oProfileID, string a_oIPAExportMethod) {
		...
		CPlatformBuilder.ExecuteJenkinsBuild(a_eBuildType, KCEditorDefine.B_JENKINS_IOS_PIPELINE, KCEditorDefine.B_IOS_BUILD_PROJ_N_JENKINS, KCDefine.B_PLATFORM_N_IOS, a_oBuildMode, oBuildVer, a_oBuildFunc, a_oPipelineName, oDataList);
	}
	...
	public static void ExecuteAndroidJenkinsBuild(EBuildType a_eBuildType, EAndroidType a_eType, string a_oBuildMode, string a_oBuildFunc, string a_oPipelineName, string a_oBuildFileExtension) {
		...
		CPlatformBuilder.ExecuteJenkinsBuild(a_eBuildType, KCEditorDefine.B_JENKINS_ANDROID_PIPELINE, KCEditorDefine.B_ANDROID_BUILD_PROJ_N_JENKINS, oPlatform, a_oBuildMode, oBuildVer, a_oBuildFunc, a_oPipelineName, oDataList);
	}
	#endregion			// 클래스 함수
}
