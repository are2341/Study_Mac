...
public class CUnityMsgSender : CSingleton<CUnityMsgSender> {
	...
	public void SendGetStoreVerMsg(string a_oAppID, string a_oVer, float a_fTimeout, System.Action<string, string> a_oCallback) {
		...
		this.ExLateCallFunc((a_oSender, a_oParams) => { 
			...
			oMsg.Add(KCDefine.U_KEY_DEVICE_MR_RESULT, KCDefine.B_FALSE_STR);
	...
	public void SendMailMsg(string a_oRecipient, string a_oTitle, string a_oMsg) {
		...
		string oVer = CAccess.GetVerStr(CProjInfoTable.Inst.ProjInfo.m_stBuildVer.m_oVer, CCommonUserInfoStorage.Inst.UserInfo.UserType);
		...
		var oDataList = new Dictionary<string, string>() {
			...
			[KCDefine.U_KEY_UNITY_MS_MAIL_MSG] = string.Format(KCDefine.B_MAIL_MSG_FMT, oApp, oVer, oPlatform, SystemInfo.processorType, SystemInfo.graphicsDeviceName, SystemInfo.graphicsDeviceType, SystemInfo.operatingSystem, oUserID, a_oMsg)
		};

		this.SendUnityMsg(KCDefine.B_CMD_MAIL, oDataList.ExToJSONStr(), null);
	}
	...
	public void SendShareMsg(string a_oMsg, System.Action<NativeShare.ShareResult, string> a_oCallback, string a_oFilePath = KCDefine.B_EMPTY_STR) {
		...
		oShare.Share();
#else
		this.ExLateCallFunc((a_oSender, a_oParams) => a_oCallback?.Invoke(NativeShare.ShareResult.NotShared, KCDefine.B_UNKNOWN_ERROR_MSG));
