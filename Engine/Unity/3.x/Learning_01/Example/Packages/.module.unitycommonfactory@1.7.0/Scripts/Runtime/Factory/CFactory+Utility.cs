...
public static partial class CFactory {
	...
	public static Gradient MakeGradient(Color a_stColor) {
		return new Gradient() {
			colorKeys = new GradientColorKey[] {
				new GradientColorKey(a_stColor, KCDefine.B_VAL_0_FLT),
				new GradientColorKey(a_stColor, KCDefine.B_VAL_1_FLT)
			},

			alphaKeys = new GradientAlphaKey[] {
				new GradientAlphaKey(a_stColor.a, KCDefine.B_VAL_0_FLT),
				new GradientAlphaKey(a_stColor.a, KCDefine.B_VAL_1_FLT)
			}
		};
	}

	//! 시퀀스를 생성한다
	public static Sequence MakeSequence(Tween a_oAni, System.Action<Sequence> a_oCallback, float a_fDelay = KCDefine.B_VAL_0_FLT, Ease a_eEase = KCDefine.U_EASE_ANI, bool a_bIsJoin = false, bool a_bIsRealtime = false) {
	...
	//! 시퀀스를 생성한다
	public static Sequence MakeSequence(List<Tween> a_oAniList, System.Action<Sequence> a_oCallback, float a_fDelay = KCDefine.B_VAL_0_FLT, Ease a_eEase = KCDefine.U_EASE_ANI, bool a_bIsJoin = false, bool a_bIsRealtime = false) {
		...
		var oDelaySequence = DOTween.Sequence().SetAutoKill().SetDelay(a_fDelay).SetUpdate(a_bIsRealtime);
	...
	public static ObjectPool CreateObjsPool(string a_oObjPath, GameObject a_oParent, int a_nNumObjs = KCDefine.U_SIZE_OBJS_POOL) {
		...
		return CFactory.CreateObjsPool(oObj, a_oParent, a_nNumObjs);
	}
