...
public class CEditorScrollerCellView : CScrollerCellView {
	...
	private void OnTouchSelBtn() {
		m_stCallbackParams.m_stBaseCallbackParams.m_oSelCallback?.Invoke(this, m_stParams.m_nLevelID);
	}

	//! 복사 버튼을 눌렀을 경우
	private void OnTouchCopyBtn() {
		m_stCallbackParams.m_oCopyCallback?.Invoke(this, m_stParams.m_nLevelID);
	}

	//! 이동 버튼을 눌렀을 경우
	private void OnTouchMoveBtn() {
		m_stCallbackParams.m_oMoveCallback?.Invoke(this, m_stParams.m_nLevelID);
	}

	//! 제거 버튼을 눌렀을 경우
	private void OnTouchRemoveBtn() {
		m_stCallbackParams.m_oRemoveCallback?.Invoke(this, m_stParams.m_nLevelID);
	}
	#endregion			// 함수
}
