...
public class CSceneLoader : CSingleton<CSceneLoader> {
	#region 함수
	//! 씬을 로드한다
	public void LoadScene(string a_oName, bool a_bIsShowIndicator = false, bool a_bIsAni = true, bool a_bIsUseLoadingScene = false, float a_fDuration = KCDefine.U_DURATION_SCREEN_FADE_IN_ANI, LoadSceneMode a_eLoadSceneMode = LoadSceneMode.Single) {
		// 인디케이터 출력 모드 일 경우
	...
	//! 씬을 로드한다
	public void LoadSceneAsync(string a_oName, System.Action<AsyncOperation, bool> a_oCallback, float a_fDelay = KCDefine.B_VAL_0_FLT, bool a_bIsShowIndicator = false, bool a_bIsAni = true, bool a_bIsUseLoadingScene = true, float a_fDuration = KCDefine.U_DURATION_SCREEN_FADE_IN_ANI, LoadSceneMode a_eLoadSceneMode = LoadSceneMode.Single) {
		// 인디케이터 출력 모드 일 경우
		if(a_bIsShowIndicator) {
			CIndicatorManager.Inst.Show(true);
		}
	...
	public void LoadAdditiveScene(string a_oName, bool a_bIsShowIndicator = false) {
		CSceneLoader.Inst.LoadScene(a_oName, a_bIsShowIndicator, false, false, KCDefine.B_VAL_0_FLT, LoadSceneMode.Additive);
	}
	...
	//! 씬을 제거한다
	public void UnloadSceneAsync(string a_oName, System.Action<AsyncOperation, bool> a_oCallback, float a_fDelay = KCDefine.B_VAL_0_FLT, bool a_bIsShowIndicator = false) {
		// 인디케이터 출력 모드 일 경우
		if(a_bIsShowIndicator) {
			CIndicatorManager.Inst.Show(true);
		}
