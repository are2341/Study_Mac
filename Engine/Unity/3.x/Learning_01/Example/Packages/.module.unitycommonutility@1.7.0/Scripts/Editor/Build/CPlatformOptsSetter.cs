...
public static partial class CPlatformOptsSetter {
	...
	public static void EditorInitialize() {
		...
		if(CPlatformOptsSetter.DefineSymbolTable != null) {
			...
			CPlatformOptsSetter.DefineSymbolDictContainer.Add(BuildTargetGroup.Standalone, CPlatformOptsSetter.DefineSymbolTable.StandaloneDefineSymbolList);
			CPlatformOptsSetter.DefineSymbolDictContainer.Add(BuildTargetGroup.iOS, CPlatformOptsSetter.DefineSymbolTable.iOSDefineSymbolList);
			CPlatformOptsSetter.DefineSymbolDictContainer.Add(BuildTargetGroup.Android, CPlatformOptsSetter.DefineSymbolTable.AndroidDefineSymbolList);

			// 배치 모드 일 경우
			if(Application.isBatchMode) {
				// 윈도우즈 일 경우
				if(CPlatformBuilder.StandaloneType == EStandaloneType.WNDS) {
					CPlatformOptsSetter.DefineSymbolDictContainer[BuildTargetGroup.Standalone].ExAddVals(CPlatformOptsSetter.DefineSymbolTable.EditorWndsDefineSymbolList);
				} else {
					CPlatformOptsSetter.DefineSymbolDictContainer[BuildTargetGroup.Standalone].ExAddVals(CPlatformOptsSetter.DefineSymbolTable.EditorMacDefineSymbolList);
				}

				// 원 스토어 일 경우
				if(CPlatformBuilder.AndroidType == EAndroidType.ONE_STORE) {
					CPlatformOptsSetter.DefineSymbolDictContainer[BuildTargetGroup.Android].ExAddVals(CPlatformOptsSetter.DefineSymbolTable.EditorOneStoreDefineSymbolList);
				}
				// 갤럭시 스토어 일 경우
				else if(CPlatformBuilder.AndroidType == EAndroidType.GALAXY_STORE) {
					CPlatformOptsSetter.DefineSymbolDictContainer[BuildTargetGroup.Android].ExAddVals(CPlatformOptsSetter.DefineSymbolTable.EditorGalaxyStoreDefineSymbolList);
				} else {
					CPlatformOptsSetter.DefineSymbolDictContainer[BuildTargetGroup.Android].ExAddVals(CPlatformOptsSetter.DefineSymbolTable.EditorGoogleDefineSymbolList);
				}
			}
	...
	public static bool IsContainsDefineSymbol(BuildTargetGroup a_eTargetGroup, string a_oDefineSymbol) {
		bool bIsContains = CPlatformOptsSetter.DefineSymbolDictContainer.ContainsKey(a_eTargetGroup);
		return bIsContains && CPlatformOptsSetter.DefineSymbolDictContainer[a_eTargetGroup].Contains(a_oDefineSymbol);
	}
	...
	public static void AddDefineSymbol(BuildTargetGroup a_eTargetGroup, string a_oDefineSymbol) {
		...
		// 전처리기 심볼 추가가 가능 할 경우
		if(CPlatformOptsSetter.DefineSymbolDictContainer.ContainsKey(a_eTargetGroup)) {
			CPlatformOptsSetter.DefineSymbolDictContainer[a_eTargetGroup].ExAddVal(a_oDefineSymbol);
		}
	}
	...
	public static void RemoveDefineSymbol(BuildTargetGroup a_eTargetGroup, string a_oDefineSymbol) {
		// 전처리기 심볼 제거가 가능 할 경우
		if(CPlatformOptsSetter.DefineSymbolDictContainer.ContainsKey(a_eTargetGroup)) {
			CPlatformOptsSetter.DefineSymbolDictContainer[a_eTargetGroup].ExRemoveVal(a_oDefineSymbol);
		}
	}
