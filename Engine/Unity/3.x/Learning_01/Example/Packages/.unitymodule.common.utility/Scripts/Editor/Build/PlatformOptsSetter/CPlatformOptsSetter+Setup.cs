...
public static partial class CPlatformOptsSetter {
	...
	public static void SetupPlayerOpts() {
		CPlatformOptsSetter.SetupPlatformOpts();
	...
	public static void SetupEditorOpts() {
		...
		VersionControlSettings.mode = KCEditorDefine.B_EDITOR_OPTS_VERSION_CONTROL;

		// 에디터 옵션을 설정한다 {
		EditorSettings.assetNamingUsesSpace = true;
		EditorSettings.enterPlayModeOptionsEnabled = false;
		EditorSettings.gameObjectNamingDigits = KCDefine.B_VALUE_INT_1 + KCDefine.B_VALUE_INT_1;
		...
		if(oSndManager != null && CPlatformOptsSetter.BuildOptsTable != null) {
			...
			oSerializeObj.ExSetPropertyValue(KCEditorDefine.B_PROPERTY_N_SND_M_DISABLE_AUDIO, (a_oProperty) => a_oProperty.boolValue = CPlatformOptsSetter.BuildOptsTable.SndOpts.m_bIsDisable);
			oSerializeObj.ExSetPropertyValue(KCEditorDefine.B_PROPERTY_N_SND_M_VIRTUALIZE_EFFECT, (a_oProperty) => a_oProperty.boolValue = CPlatformOptsSetter.BuildOptsTable.SndOpts.m_bIsVirtualizeEffect);
			oSerializeObj.ExSetPropertyValue(KCEditorDefine.B_PROPERTY_N_SND_M_GLOBAL_VOLUME, (a_oProperty) => a_oProperty.floatValue = CPlatformOptsSetter.BuildOptsTable.SndOpts.m_fGlobalVolume);
			oSerializeObj.ExSetPropertyValue(KCEditorDefine.B_PROPERTY_N_SND_M_ROLLOFF_SCALE, (a_oProperty) => a_oProperty.floatValue = CPlatformOptsSetter.BuildOptsTable.SndOpts.m_fRolloffScale);
			oSerializeObj.ExSetPropertyValue(KCEditorDefine.B_PROPERTY_N_SND_M_DOPPLER_FACTOR, (a_oProperty) => a_oProperty.floatValue = CPlatformOptsSetter.BuildOptsTable.SndOpts.m_fDopplerFactor);
		}
		// 사운드 옵션을 설정한다 }
		...
		if(oTagManager != null) {
			...
			// 태그를 설정한다
			oSerializeObj.ExSetPropertyValue(KCEditorDefine.B_PROPERTY_N_TAG_M_TAG, (a_oProperty) => {
#if !EXTRA_TAG_ENABLE
			...
			// 정렬 레이어를 설정한다
			oSerializeObj.ExSetPropertyValue(KCEditorDefine.B_PROPERTY_N_TAG_M_SORTING_LAYER, (a_oProperty) => {
				...
				for(int i = 0; i < a_oProperty.arraySize; ++i) {
					...
					while(oEnumerator.MoveNext()) {
						...
						// 고유 식별자 이름과 동일 할 경우
							...
							oProperty.intValue = oSortingLayer.ExIsEquals(KCDefine.U_SORTING_L_DEF) ? KCDefine.B_VALUE_INT_0 : i + KCEditorDefine.B_UNIT_CUSTOM_TAG_ID;
						}
					}
				}
			});
	...
	public static void SetupPluginProjs() {
		...
		CFunc.CopyFile(KCEditorDefine.B_ORIGIN_SRC_PROGUARD_P_ANDROID, KCEditorDefine.B_SRC_PROGUARD_P_ANDROID, false);
		CFunc.CopyFile(KCEditorDefine.B_SRC_PROGUARD_P_ANDROID, KCEditorDefine.B_DEST_PROGUARD_P_ANDROID, false);
	...
	public static void SetupDefineSymbols() {
		...
		if(CPlatformOptsSetter.DefineSymbolListContainer.ExIsValid()) {
			foreach(var stKeyValue in CPlatformOptsSetter.DefineSymbolListContainer) {
				CPlatformOptsSetter.RemoveDefineSymbol(stKeyValue.Key, KCEditorDefine.DS_DEFINE_S_FPS_ENABLE);
				CPlatformOptsSetter.RemoveDefineSymbol(stKeyValue.Key, KCEditorDefine.DS_DEFINE_S_ADS_TEST_ENABLE);
				CPlatformOptsSetter.RemoveDefineSymbol(stKeyValue.Key, KCEditorDefine.DS_DEFINE_S_LOGIC_TEST_ENABLE);
				CPlatformOptsSetter.RemoveDefineSymbol(stKeyValue.Key, KCEditorDefine.DS_DEFINE_S_RECEIPT_CHECK_ENABLE);
				CPlatformOptsSetter.RemoveDefineSymbol(stKeyValue.Key, KCEditorDefine.DS_DEFINE_S_ADHOC_BUILD);
				CPlatformOptsSetter.RemoveDefineSymbol(stKeyValue.Key, KCEditorDefine.DS_DEFINE_S_STORE_BUILD);
			}

			CEditorFunc.SetupDefineSymbols(CPlatformOptsSetter.DefineSymbolListContainer);
		}
		// 전처리기 심볼을 설정한다 }
	}
	...
	//! 플랫폼 옵션을 설정한다
	private static void SetupPlatformOpts() {
		CPlatformOptsSetter.EditorInitialize();
		...
		if(CPlatformOptsSetter.ProjInfoTable != null) {
			PlayerSettings.macOS.buildNumber = CPlatformOptsSetter.ProjInfoTable.MacProjInfo.m_stBuildVersion.m_nNumber.ToString();
			PlayerSettings.iOS.buildNumber = CPlatformOptsSetter.ProjInfoTable.iOSProjInfo.m_stBuildVersion.m_nNumber.ToString();
			...
			if(Application.isBatchMode) {
				...
				if(CPlatformBuilder.AndroidType == EAndroidType.ONE_STORE) {
					bIsValidNumber = int.TryParse(CPlatformOptsSetter.ProjInfoTable.OneStoreProjInfo.m_stBuildVersion.m_nNumber.ToString(), out nBuildNumber);
				}
				// 갤럭시 스토어 일 경우
				else if(CPlatformBuilder.AndroidType == EAndroidType.GALAXY_STORE) {
					bIsValidNumber = int.TryParse(CPlatformOptsSetter.ProjInfoTable.GalaxyStoreProjInfo.m_stBuildVersion.m_nNumber.ToString(), out nBuildNumber);
				} else {
					bIsValidNumber = int.TryParse(CPlatformOptsSetter.ProjInfoTable.GoogleProjInfo.m_stBuildVersion.m_nNumber.ToString(), out nBuildNumber);
				}
			} else {
#if WNDS
				PlayerSettings.SetApplicationIdentifier(BuildTargetGroup.Standalone, CPlatformOptsSetter.ProjInfoTable.WndsProjInfo.m_oAppID);
			...
			CAccess.Assert(bIsValidNumber && nBuildNumber >= KCDefine.B_MIN_BUILD_NUMBER);
	...
	private static void SetupStandaloneOpts() {
		...
		if(CPlatformOptsSetter.BuildInfoTable != null) {
			...
			CAccessExtension.ExSetRuntimePropertyValue<PlayerSettings.macOS>(null, KCEditorDefine.B_PROPERTY_N_CATEGORY, stBuildInfo.m_oCategory);
		}
	}
	...
	private static void SetupBuildOpts() {
		...
		if(CPlatformOptsSetter.BuildOptsTable != null) {
			...
			PlayerSettings.SetMobileMTRendering(BuildTargetGroup.iOS, 
				CPlatformOptsSetter.BuildOptsTable.CommonBuildOpts.m_bIsMTRendering);
	...
	private static void SetupiOSBuildOpts() {
		...
		if(CPlatformOptsSetter.BuildInfoTable != null) {
			...
			PlayerSettings.iOS.targetOSVersionString = CPlatformOptsSetter.BuildInfoTable.iOSBuildInfo.m_oTargetOSVersion;
		...
		if(CPlatformOptsSetter.BuildOptsTable != null) {
			...
			CAccessExtension.ExSetPropertyValue<PlayerSettings.iOS>(null, KCEditorDefine.B_PROPERTY_N_APPLE_ENABLE_PRO_MOTION, KCDefine.B_BINDING_F_NON_PUBLIC_STATIC, stBuildOpts.m_bIsEnableProMotion);
			CAccessExtension.ExSetPropertyValue<PlayerSettings.iOS>(null, KCEditorDefine.B_PROPERTY_N_REQUIRE_AR_KIT_SUPPORTS, KCDefine.B_BINDING_F_NON_PUBLIC_STATIC, stBuildOpts.m_bIsRequreARKitSupports);
			CAccessExtension.ExSetPropertyValue<PlayerSettings.iOS>(null, KCEditorDefine.B_PROPERTY_N_AUTO_ADD_CAPABILITIES, KCDefine.B_BINDING_F_NON_PUBLIC_STATIC, stBuildOpts.m_bIsAutoAddCapabilities);
		}
	}
	...
	private static void SetupAndroidBuildOpts() {
		...
		CAccessExtension.ExSetPropertyValue<PlayerSettings.Android>(null, KCEditorDefine.B_PROPERTY_N_VALIDATE_APP_BUNDLE_SIZE, KCDefine.B_BINDING_F_NON_PUBLIC_STATIC, true);
		...
		// 빌드 정보 테이블이 존재 할 경우
		if(CPlatformOptsSetter.BuildInfoTable != null) {
			PlayerSettings.Android.minSdkVersion = CPlatformOptsSetter.BuildInfoTable.AndroidBuildInfo.m_eMinSDKVersion;
			PlayerSettings.Android.targetSdkVersion = CPlatformOptsSetter.BuildInfoTable.AndroidBuildInfo.m_eTargetSDKVersion;
		}
		...
		if(CPlatformOptsSetter.BuildOptsTable != null) {
			...
			CAccessExtension.ExSetPropertyValue<PlayerSettings.Android>(null, KCEditorDefine.B_PROPERTY_N_APP_BUNDLE_SIZE_TO_VALIDATE, KCDefine.B_BINDING_F_NON_PUBLIC_STATIC, stBuildOpts.m_nAppBundleSize);
			CAccessExtension.ExSetPropertyValue<PlayerSettings.Android>(null, KCEditorDefine.B_PROPERTY_N_SUPPORTED_ASPECT_RATIO_MODE, KCDefine.B_BINDING_F_NON_PUBLIC_STATIC, (int)stBuildOpts.m_eAspectRatioMode);
		}
	}
	#endregion			// 클래스 함수
}
