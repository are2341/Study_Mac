...
public class CDeviceMsgReceiver : CSingleton<CDeviceMsgReceiver> {
	#region 변수
	private Dictionary<string, System.Action<string, string>> m_oCallbackList = new Dictionary<string, System.Action<string, string>>();
	#endregion			// 변수

	#region 함수
	//! 콜백을 추가한다
	public void AddCallback(string a_oKey, System.Action<string, string> a_oCallback) {
		m_oCallbackList.ExAddValue(a_oKey, a_oCallback);
	}

	//! 콜백을 제거한다
	public void RemoveCallback(string a_oKey) {
		m_oCallbackList.ExRemoveValue(a_oKey);
	}
	...
	private void HandleDeviceMsg(string a_oMsg) {
		...
		CScheduleManager.Inst.AddCallback(oKey, () => {
			CFunc.ShowLog("CDeviceMsgReceiver.HandleDeviceMsg: {0}", KCDefine.B_LOG_COLOR_PLUGIN, a_oMsg);

			// 커맨드가 존재 할 경우
			if(m_oCallbackList.ContainsKey(oCmd)) {
				var oCallback = m_oCallbackList[oCmd];
