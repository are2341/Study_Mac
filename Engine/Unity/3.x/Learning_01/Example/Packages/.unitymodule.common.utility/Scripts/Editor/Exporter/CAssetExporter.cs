...
public static class CAssetExporter {
	...
	public static void ExportSpriteToPNGImg() {
		...
		if(!oSpriteList.ExIsValid()) {
			...
			for(int i = 0; i < oSpriteList.Count; ++i) {
				var oTexture = new Texture2D((int)oSpriteList[i].textureRect.width, (int)oSpriteList[i].textureRect.height, TextureFormat.RGBA32, KCDefine.B_VALUE_INT_1, true);
				var oColors = oSpriteList[i].texture.GetPixels((int)oSpriteList[i].textureRect.x, (int)oSpriteList[i].textureRect.y, (int)oSpriteList[i].textureRect.width, (int)oSpriteList[i].textureRect.height, KCDefine.B_VALUE_INT_0);
	...
	private static void SaveTexture(string a_oFilePath, Texture2D a_oTexture) {
		var oTexture = new Texture2D(a_oTexture.width, a_oTexture.height, TextureFormat.RGBA32, KCDefine.B_VALUE_INT_1, true);
		oTexture.SetPixels(a_oTexture.GetPixels(KCDefine.B_VALUE_INT_0));
		oTexture.Apply();
