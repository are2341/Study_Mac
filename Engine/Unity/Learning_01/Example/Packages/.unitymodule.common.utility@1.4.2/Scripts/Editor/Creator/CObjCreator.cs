...
public static class CObjCreator {
	...
	//! 재사용 스크롤 뷰를 생성한다
	[MenuItem("GameObject/Utility/UI/ScrollView/RecycleScrollView", false, 1)]
	...
	//! 페이지 스크롤 뷰를 생성한다
	[MenuItem("GameObject/Utility/UI/ScrollView/PageScrollView", false, 1)]
	...
	private static GameObject CreateObj(string a_oName, string a_oFilePath, bool a_bIsIgnoreParent = false) {
		...
		if(oObj.TryGetComponent<Button>(out Button oBtn)) {
			...
			stColorBlock.selectedColor = KCDefine.U_COLOR_SELECT;
		...
		// 부모 객체가 존재 할 경우
		if(oParent != null && oParent.TryGetComponent<RectTransform>(out RectTransform oTrans)) {
