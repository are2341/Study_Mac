...
public class CSndManager : CSingleton<CSndManager> {
	...
	private Dictionary<string, int> m_oMaxNumDuplicateFXSndsList = new Dictionary<string, int>();
	...
	//! 최대 중첩 효과음 개수를 설정한다
	public void SetupMaxNumDuplicateFXSnds(string a_oKey, int a_nNumDuplicateFXSnds) {
		CAccess.Assert(a_oKey.ExIsValid());
		m_oMaxNumDuplicateFXSndsList.ExReplaceVal(a_oKey, a_nNumDuplicateFXSnds);
	}
	...
	private CSnd FindPlayableFXSnd(string a_oKey) {
		...
		var oFXSndList = m_oFXSndListContainer.ExGetVal(a_oKey, null);
		int nMaxNumDuplicateFXSnds = m_oMaxNumDuplicateFXSndsList.ExGetVal(a_oKey, KCDefine.U_MAX_NUM_DUPLICATE_FX_SNDS);
		
		// 효과음이 없을 경우
		if(oFXSndList == null) {
			oFXSndList = new List<CSnd>();
			m_oFXSndListContainer.ExAddVal(a_oKey, oFXSndList);
		}
	...
	private void EnumerateFXSnds(System.Action<string, CSnd> a_oCallback) {
		foreach(var stKeyVal in m_oFXSndListContainer) {
