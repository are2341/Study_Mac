...
public class CValTable : CSingleton<CValTable> {
	...
	//! 값을 로드한다
	public List<object> LoadVals(string a_oCSVStr) {
	...
	//! 값을 로드한다
	public List<object> LoadValsFromFile(string a_oFilePath) {
		CAccess.Assert(a_oFilePath.ExIsValid());
		string oCSVStr = CFunc.ReadStr(a_oFilePath);

		return this.LoadVals(oCSVStr);
	}
	...
	public List<object> LoadValsFromRes(string a_oFilePath) {
		...
		try {
			...
			return this.LoadVals(oTextAsset.text);
		} finally {
