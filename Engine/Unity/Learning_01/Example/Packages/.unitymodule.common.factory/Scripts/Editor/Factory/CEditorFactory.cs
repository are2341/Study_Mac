...
public static class CEditorFactory {
	...
	#region 제네릭 클래스 함수
	//! 에디터 윈도우를 생성한다
	public static T CreateEditorWnd<T>(string a_oName, Vector2 a_stMinSize) where T : EditorWindow {
	...
	//! 스크립트 객체를 생성한다
	public static T CreateScriptableObj<T>(string a_oFilePath = KCDefine.B_EMPTY_STRING) where T : ScriptableObject {
