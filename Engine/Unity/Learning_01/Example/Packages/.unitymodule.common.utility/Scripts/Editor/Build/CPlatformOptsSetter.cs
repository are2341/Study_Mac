...
public static partial class CPlatformOptsSetter {
	...
	public static void EditorInitialize() {
		...
		if(CPlatformOptsSetter.DefineSymbolTable != null) {
			...
			CPlatformOptsSetter.DefineSymbolListContainer.ExAddValue(BuildTargetGroup.Standalone, CPlatformOptsSetter.DefineSymbolTable.StandaloneDefineSymbolList);
			CPlatformOptsSetter.DefineSymbolListContainer.ExAddValue(BuildTargetGroup.iOS, CPlatformOptsSetter.DefineSymbolTable.iOSDefineSymbolList);
			CPlatformOptsSetter.DefineSymbolListContainer.ExAddValue(BuildTargetGroup.Android, CPlatformOptsSetter.DefineSymbolTable.AndroidDefineSymbolList);

			// 배치 모드 일 경우
			if(Application.isBatchMode) {
				// 윈도우즈 일 경우
				if(CPlatformBuilder.StandaloneType == EStandaloneType.WNDS) {
					CPlatformOptsSetter.DefineSymbolListContainer.ExAddValue(BuildTargetGroup.Standalone, CPlatformOptsSetter.DefineSymbolTable.EditorWndsDefineSymbolList);
				} else {
					CPlatformOptsSetter.DefineSymbolListContainer.ExAddValue(BuildTargetGroup.Standalone, CPlatformOptsSetter.DefineSymbolTable.EditorMacDefineSymbolList);
				}

				// 원 스토어 일 경우
				if(CPlatformBuilder.AndroidType == EAndroidType.ONE_STORE) {
					CPlatformOptsSetter.DefineSymbolListContainer.ExAddValue(BuildTargetGroup.Android, CPlatformOptsSetter.DefineSymbolTable.EditorOneStoreDefineSymbolList);
				}
				// 갤럭시 스토어 일 경우
				else if(CPlatformBuilder.AndroidType == EAndroidType.GALAXY_STORE) {
					CPlatformOptsSetter.DefineSymbolListContainer.ExAddValue(BuildTargetGroup.Android, CPlatformOptsSetter.DefineSymbolTable.EditorGalaxyStoreDefineSymbolList);
				} else {
					CPlatformOptsSetter.DefineSymbolListContainer.ExAddValue(BuildTargetGroup.Android, CPlatformOptsSetter.DefineSymbolTable.EditorGoogleDefineSymbolList);
				}
			}

			foreach(var stKeyValue in CPlatformOptsSetter.DefineSymbolListContainer) {
				bool bIsContainsStudy = stKeyValue.Value.Contains(KCEditorDefine.DS_DEFINE_S_STUDY_ENABLE);
				bool bIsContainsPurchase = stKeyValue.Value.Contains(KCEditorDefine.DS_DEFINE_S_PURCHASE_ENABLE);

				// 스터디 전처리기 심볼이 존재 할 경우
				if(bIsContainsStudy || stKeyValue.Value.Contains(KCEditorDefine.DS_DEFINE_S_STUDY_MODULE_ENABLE)) {
					stKeyValue.Value.ExAddValue(KCEditorDefine.DS_DEFINE_S_LIGHT_ENABLE);
					stKeyValue.Value.ExAddValue(KCEditorDefine.DS_DEFINE_S_SHADOW_ENABLE);
					stKeyValue.Value.ExAddValue(KCEditorDefine.DS_DEFINE_S_SOFT_SHADOW_ENABLE);
					stKeyValue.Value.ExAddValue(KCEditorDefine.DS_DEFINE_S_LIGHTMAP_SHADOW_BAKE_MASK_MODE_ENABLE);
					stKeyValue.Value.ExAddValue(KCEditorDefine.DS_DEFINE_S_ULTRA_QUALITY_LEVEL_ENABLE);
				}
				// 결제 전처리기 심볼이 존재 할 경우
				else if(bIsContainsPurchase || stKeyValue.Value.Contains(KCEditorDefine.DS_DEFINE_S_PURCHASE_MODULE_ENABLE)) {
					stKeyValue.Value.ExAddValue(KCEditorDefine.DS_DEFINE_S_UNITY_PURCHASING);
				}

				stKeyValue.Value.ExAddValue(KCEditorDefine.DS_DEFINE_S_IL2CPP_ENABLE);
				stKeyValue.Value.ExAddValue(KCEditorDefine.DS_DEFINE_S_USE_CUSTOM_PROJ_OPTS);
				stKeyValue.Value.ExAddValue(KCEditorDefine.DS_DEFINE_S_ODIN_INSPECTOR);
				stKeyValue.Value.ExAddValue(KCEditorDefine.DS_DEFINE_S_ODIN_INSPECTOR_3);
			}
		}
	}
	...
	public static void AddDefineSymbol(BuildTargetGroup a_eTargetGroup, string a_oDefineSymbol) {
		...
		if(CPlatformOptsSetter.DefineSymbolListContainer.ContainsKey(a_eTargetGroup)) {
			CPlatformOptsSetter.DefineSymbolListContainer[a_eTargetGroup].ExAddValue(a_oDefineSymbol);
		}
	}
	...
	public static void RemoveDefineSymbol(BuildTargetGroup a_eTargetGroup, string a_oDefineSymbol) {
		// 전처리기 심볼 제거가 가능 할 경우
		if(CPlatformOptsSetter.DefineSymbolListContainer.ContainsKey(a_eTargetGroup)) {
			CPlatformOptsSetter.DefineSymbolListContainer[a_eTargetGroup].ExRemoveValue(a_oDefineSymbol);
		}
	}
