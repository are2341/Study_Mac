...
public static partial class CFactory {
	...
	public static string MakeLocalizePath(string a_oBasePath, string a_oLanguage) {
		...
		return a_oBasePath.ExGetReplaceFileNamePath(string.Format(KCDefine.B_TEXT_FMT_2_UNDER_SCORE_COMBINE, oFileName, a_oLanguage));
	}
	...
	private static void ExSwap<T>(this List<T> a_oSender, int a_nIdxA, int a_nIdxB, bool a_bIsEnableAssert = true) {
		CAccess.Assert(!a_bIsEnableAssert || (a_oSender != null && a_oSender.ExIsValidIdx(a_nIdxA) && a_oSender.ExIsValidIdx(a_nIdxB)));
		
		// 인덱스가 유효 할 경우
		if(a_oSender != null && (a_oSender.ExIsValidIdx(a_nIdxA) && a_oSender.ExIsValidIdx(a_nIdxB))) {
