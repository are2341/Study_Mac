...
public static partial class CPlatformOptsSetter {
	...
	public static void SetupProjOpts() {
		...
		for(int i = 0; i < KCEditorDefine.B_SCRIPT_P_INFO_LIST.Count; ++i) {
			CFunc.CopyFile(KCEditorDefine.B_SCRIPT_P_INFO_LIST[i].Item1, KCEditorDefine.B_SCRIPT_P_INFO_LIST[i].Item2, KCEditorDefine.DS_DEFINE_S_NEVER_USE_THIS, System.Text.Encoding.UTF8, false);
		}
	...
	public static void SetupQuality() {
		...
		for(int i = 0; i < oQualityLevelList.Count; ++i) {
			// 퀄리티 수준이 다를 경우
			if(oQualityLevelList[i] != CAccess.QualityLevel) {
				CPlatformOptsSetter.DoSetupQuality(oQualityLevelList[i]);
			}
		}

		CPlatformOptsSetter.DoSetupQuality(CAccess.QualityLevel);
	}
		...
		if(CPlatformOptsSetter.ProjInfoTable != null) {
			...
			if(Application.isBatchMode) {
				...
				switch(CPlatformBuilder.AndroidType) {
					...
					case EAndroidType.ONE_STORE: PlayerSettings.Android.bundleVersionCode = CPlatformOptsSetter.ProjInfoTable.AndroidOneStoreProjInfo.m_stBuildVerInfo.m_nNum; PlayerSettings.SetApplicationIdentifier(BuildTargetGroup.Android, CPlatformOptsSetter.ProjInfoTable.AndroidOneStoreProjInfo.m_oAppID); break;
				...
				switch(CPlatformBuilder.StandaloneType) {
					case EStandaloneType.MAC_STEAM: PlayerSettings.macOS.buildNumber = $"{CPlatformOptsSetter.ProjInfoTable.StandaloneMacSteamProjInfo.m_stBuildVerInfo.m_nNum}"; PlayerSettings.SetApplicationIdentifier(BuildTargetGroup.Standalone, CPlatformOptsSetter.ProjInfoTable.StandaloneMacSteamProjInfo.m_oAppID); break;
					...
					default: PlayerSettings.macOS.buildNumber = $"{CPlatformOptsSetter.ProjInfoTable.StandaloneMacAppleProjInfo.m_stBuildVerInfo.m_nNum}"; PlayerSettings.SetApplicationIdentifier(BuildTargetGroup.Standalone, CPlatformOptsSetter.ProjInfoTable.StandaloneMacAppleProjInfo.m_oAppID); break;
				}
			} else {
				...
				PlayerSettings.Android.bundleVersionCode = CPlatformOptsSetter.ProjInfoTable.AndroidOneStoreProjInfo.m_stBuildVerInfo.m_nNum; PlayerSettings.SetApplicationIdentifier(BuildTargetGroup.Android, CPlatformOptsSetter.ProjInfoTable.AndroidOneStoreProjInfo.m_oAppID);
#else
...
#else
				PlayerSettings.macOS.buildNumber = $"{CPlatformOptsSetter.ProjInfoTable.StandaloneMacAppleProjInfo.m_stBuildVerInfo.m_nNum}"; PlayerSettings.SetApplicationIdentifier(BuildTargetGroup.Standalone, CPlatformOptsSetter.ProjInfoTable.StandaloneMacAppleProjInfo.m_oAppID);
	...
	private static void SetupiOSBuildOpts() {
		...
		CAccessExtension.ExSetPropertyVal<PlayerSettings.iOS>(null, KCEditorDefine.B_PROPERTY_N_APPLE_ENABLE_PRO_MOTION, KCDefine.B_BINDING_F_NON_PUBLIC_STATIC, false);
...
#if UNIVERSAL_RENDERING_PIPELINE_MODULE_ENABLE
		CFunc.SetupQuality(a_eQualityLevel, true);
...
#else
		CFunc.SetupQuality(a_eQualityLevel, false);
#endif			// #if UNIVERSAL_RENDERING_PIPELINE_MODULE_ENABLE
	}
	#endregion			// 클래스 함수
	...
	private static void SetupRenderingPipeline(EQualityLevel a_eQualityLevel, UniversalRenderPipelineAsset a_oRenderingPipeline, bool a_bIsEnableAssert = true) {
		...
		if(a_oRenderingPipeline != null) {
			var oRenderingPipelineRendererDataList = new List<UniversalRendererData>() {
				Resources.Load<UniversalRendererData>(KCDefine.U_PIPELINE_P_G_UNIVERSAL_RP_RENDERER_DATA),
				Resources.Load<UniversalRendererData>(KCDefine.U_PIPELINE_P_G_UNIVERSAL_RP_SSAO_RENDERER_DATA)
			};

			var oRenderingPipeline2DRendererDataList = new List<Renderer2DData>() {
				Resources.Load<Renderer2DData>(KCDefine.U_PIPELINE_P_G_UNIVERSAL_RP_2D_RENDERER_DATA),
				Resources.Load<Renderer2DData>(KCDefine.U_PIPELINE_P_G_UNIVERSAL_RP_2D_SSAO_RENDERER_DATA)
			};
...
#if LIGHT_ENABLE
					oRenderingPipelineRendererDataList[i].shadowTransparentReceive = false;
#else
					oRenderingPipelineRendererDataList[i].shadowTransparentReceive = false;
#endif			// #if LIGHT_ENABLE
					
// TODO: 해당 기능 안정화 필요 (RenderingMode.Deferred)
// #if DEFERRED_RENDERING_ENABLE
// 					oRenderingPipelineRendererDataList[i].renderingMode = RenderingMode.Deferred;
// #else
// 					oRenderingPipelineRendererDataList[i].renderingMode = RenderingMode.Forward;
// #endif			// #if DEFERRED_RENDERING_ENABLE
				}
			}
			...
			a_oRenderingPipeline.shadowDistance = KCDefine.U_DISTANCE_CAMERA_FAR_PLANE / KCDefine.B_VAL_5_FLT;
			...
			switch(a_eQualityLevel) {
				case EQualityLevel.HIGH: {
					a_oRenderingPipeline.ExSetRuntimeFieldVal<UniversalRenderPipelineAsset>(KCEditorDefine.U_FIELD_N_UNIVERSAL_RP_ADDITIONAL_LIGHT_COOKIE_RESOLUTION, LightCookieResolution._512);
					a_oRenderingPipeline.ExSetRuntimeFieldVal<UniversalRenderPipelineAsset>(KCEditorDefine.U_FIELD_N_UNIVERSAL_RP_MAIN_LIGHT_SHADOW_MAP_RESOLUTION, UnityEngine.Rendering.Universal.ShadowResolution._2048);
					a_oRenderingPipeline.ExSetRuntimeFieldVal<UniversalRenderPipelineAsset>(KCEditorDefine.U_FIELD_N_UNIVERSAL_RP_ADDITIONAL_LIGHT_SHADOW_MAP_RESOLUTION, UnityEngine.Rendering.Universal.ShadowResolution._1024);
				} break;
				...
				default: {
					a_oRenderingPipeline.ExSetRuntimeFieldVal<UniversalRenderPipelineAsset>(KCEditorDefine.U_FIELD_N_UNIVERSAL_RP_ADDITIONAL_LIGHT_COOKIE_RESOLUTION, LightCookieResolution._256);
					a_oRenderingPipeline.ExSetRuntimeFieldVal<UniversalRenderPipelineAsset>(KCEditorDefine.U_FIELD_N_UNIVERSAL_RP_MAIN_LIGHT_SHADOW_MAP_RESOLUTION, UnityEngine.Rendering.Universal.ShadowResolution._1024);
					a_oRenderingPipeline.ExSetRuntimeFieldVal<UniversalRenderPipelineAsset>(KCEditorDefine.U_FIELD_N_UNIVERSAL_RP_ADDITIONAL_LIGHT_SHADOW_MAP_RESOLUTION, UnityEngine.Rendering.Universal.ShadowResolution._512);
				} break;
			}
			...
			switch(a_eQualityLevel) {
				...
				case EQualityLevel.ULTRA: {
					a_oRenderingPipeline.ExSetRuntimeFieldVal<UniversalRenderPipelineAsset>(KCEditorDefine.U_FIELD_N_UNIVERSAL_RP_MSAA_QUALITY, MsaaQuality._8x);
				} break;
				default: {
					a_oRenderingPipeline.ExSetRuntimeFieldVal<UniversalRenderPipelineAsset>(KCEditorDefine.U_FIELD_N_UNIVERSAL_RP_MSAA_QUALITY, MsaaQuality._2x);
				} break;
			}
#else
...
#else
			a_oRenderingPipeline.ExSetRuntimeFieldVal<UniversalRenderPipelineAsset>(KCEditorDefine.U_FIELD_N_UNIVERSAL_RP_MAIN_LIGHT_SUPPORTS_SHADOW, false);
			a_oRenderingPipeline.ExSetRuntimeFieldVal<UniversalRenderPipelineAsset>(KCEditorDefine.U_FIELD_N_UNIVERSAL_RP_ADDITIONAL_LIGHT_SUPPORTS_SHADOW, false);
#endif			// #if LIGHT_ENABLE
