...
public static partial class CPlatformBuilder {
	#region 클래스 함수
	/** iOS 젠킨스 빌드를 실행한다 */
	public static void ExecuteiOSJenkinsBuild(EiOSType a_eType, string a_oBuildMode, string a_oBuildFunc, string a_oPipelineName, string a_oBuildFileExtension, string a_oProfileID, string a_oIPAExportMethod) {
		var oDataDict = new Dictionary<string, string>() {
			[KCEditorDefine.B_KEY_JENKINS_BUNDLE_ID] = CPlatformOptsSetter.ProjInfoTable.iOSAppleProjInfo.m_oAppID, 
	...
	/** 안드로이드 젠킨스 빌드를 실행한다 */
	public static void ExecuteAndroidJenkinsBuild(EAndroidType a_eType, string a_oBuildMode, string a_oBuildFunc, string a_oPipelineName, string a_oBuildFileExtension) {
		var stProjInfo = (a_eType == EAndroidType.AMAZON) ? CPlatformOptsSetter.ProjInfoTable.AndroidGoogleProjInfo : (a_eType == EAndroidType.ONE_STORE) ? CPlatformOptsSetter.ProjInfoTable.AndroidOneStoreProjInfo : CPlatformOptsSetter.ProjInfoTable.AndroidGoogleProjInfo;
	...
	/** 독립 플랫폼 젠킨스 빌드를 실행한다 */
	public static void ExecuteStandaloneJenkinsBuild(EStandaloneType a_eType, string a_oBuildMode, string a_oBuildFunc, string a_oPipelineName, string a_oBuildFileExtension) {
		var stProjInfo = (a_eType == EStandaloneType.MAC_STEAM) ? CPlatformOptsSetter.ProjInfoTable.StandaloneMacSteamProjInfo : (a_eType == EStandaloneType.WNDS_STEAM) ? CPlatformOptsSetter.ProjInfoTable.StandaloneWndsSteamProjInfo : CPlatformOptsSetter.ProjInfoTable.StandaloneMacAppleProjInfo;
		...
		CPlatformBuilder.ExecuteJenkinsBuild(new STJenkinsParams() {
			...
			m_oProjPlatform = a_eType.ExIsWnds() ? KCEditorDefine.B_PROJ_PLATFORM_N_STANDALONE_WNDS : KCEditorDefine.B_PROJ_PLATFORM_N_STANDALONE_MAC,
			m_oBuildMode = a_oBuildMode,
			m_oBuildVer = string.Format(KCDefine.B_TEXT_FMT_2_UNDER_SCORE_COMBINE, stProjInfo.m_stBuildVerInfo.m_oVer, stProjInfo.m_stBuildVerInfo.m_nNum),
			m_oBuildFunc = a_oBuildFunc,
			m_oPipelineName = a_oPipelineName,
	...
	private static void ExecuteJenkinsBuild(STJenkinsParams a_stParams) {
		...
		oDataDict.TryAdd(KCEditorDefine.B_KEY_JENKINS_ENGINE_VER, Application.unityVersion);
		...
		oDataDict.TryAdd(KCEditorDefine.B_KEY_JENKINS_PROJ_NAME, CPlatformOptsSetter.ProjInfoTable.ProjName);
		oDataDict.TryAdd(KCEditorDefine.B_KEY_JENKINS_PROJ_PATH, string.Format(KCDefine.B_TEXT_FMT_3_SLASH_COMBINE, CPlatformOptsSetter.BuildInfoTable.JenkinsInfo.m_oWorkspace, a_stParams.m_oSrc, CPlatformOptsSetter.ProjInfoTable.ProjName));
		...
		oDataDict.TryAdd(KCEditorDefine.B_KEY_JENKINS_ANALYTICS, string.Format(KCEditorDefine.B_ANALYTICS_FMT_JENKINS, CPlatformOptsSetter.BuildInfoTable.JenkinsInfo.m_oSrc));
		oDataDict.TryAdd(KCEditorDefine.B_KEY_JENKINS_BUILD_MODE, a_stParams.m_oBuildMode);
