...
/** 터치 비율 처리자 */
public class CTouchScaler : CComponent, IPointerUpHandler, IPointerDownHandler {
	#region 변수
	...
	public void SetScale(Vector3 a_stScale, bool a_bIsAni = true) {
		...
		if(!this.IsIgnoreAni && a_bIsAni) {
			m_oScaleAni = this.transform.DOScale(a_stScale, m_fDuration);
			m_oScaleAni.SetEase(Ease.Linear).SetUpdate(true);
		} else {
