...
public class CEditorLevelCreatePopup : CPopup {
	...
	protected virtual CEditorLevelCreateInfo CreateEditorLevelCreateInfo() {
		...
		var stMinNumCells = new Vector3Int(Mathf.Min(nMinNumCellsX, nMaxNumCellsX), Mathf.Min(nMinNumCellsY, nMaxNumCellsY), KCDefine.B_VAL_0_INT);
		var stMaxNumCells = new Vector3Int(Mathf.Max(nMinNumCellsX, nMaxNumCellsX), Mathf.Max(nMinNumCellsY, nMaxNumCellsY), KCDefine.B_VAL_0_INT);

		return new CEditorLevelCreateInfo() {
			m_nNumLevels = nNumLevels, m_stMinNumCells = stMinNumCells, m_stMaxNumCells = stMaxNumCells
		};
	}
