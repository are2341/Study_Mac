...
public abstract partial class CSceneManager : CComponent {
	...
	protected virtual void SetupScene(bool a_bIsMainSetup) {
		...
		if(a_bIsMainSetup) {
			...
			if(this.MainCamera != null) {
				...
				oPostProcessingVolume?.ExSetEnable(this.IsActiveScene, false);

				// 포스트 프로세싱 볼륨이 존재 할 경우
				if(oPostProcessingVolume != null) {
					...
					bool bIsValidB = oPostProcessingVolume.sharedProfile != null && !oPostProcessingVolume.sharedProfile.name.Equals(Path.GetFileNameWithoutExtension(oPostProcessingSettingsPathDict[CAccess.QualityLevel]));
					...
					// 포스트 프로세싱 설정이 없을 경우
					if((!bIsValidA || (bIsValidB && oResult.Item1)) && CAccess.IsExistsRes<PostProcessProfile>(oPostProcessingSettingsPathDict[CAccess.QualityLevel], true)) {
						oPostProcessingVolume.sharedProfile = Resources.Load<PostProcessProfile>(oPostProcessingSettingsPathDict[CAccess.QualityLevel]);
					}
				}
#endif			// #if POST_PROCESSING_MODULE_ENABLE
			}
...
#else
			a_oLight.shadows = LightShadows.None;
	...
	/** 고정 UI 상태를 설정한다 */
	private void SetupAnchorUIsState(GameObject a_oUIsObjs, ref Vector3 a_stPos, ref Vector3 a_stSize, ref Vector3 a_stPivot, bool a_bIsEnableAssert = true) {
		CAccess.Assert(!a_bIsEnableAssert || a_oUIsObjs != null);
		...
		if(a_oUIsObjs != null) {
			...
			if(bIsUpUIs || bIsDownUIs) {
				a_stSize = new Vector3(KCDefine.B_SCREEN_WIDTH, KCDefine.B_VAL_0_FLT, KCDefine.B_VAL_0_FLT);
				a_stPivot = bIsUpUIs ? KCDefine.B_ANCHOR_UP_CENTER : KCDefine.B_ANCHOR_DOWN_CENTER;
				
				// 위쪽 UI 일 경우
				if(bIsUpUIs) {
					a_stPos = new Vector3(KCDefine.B_VAL_0_FLT, (CSceneManager.CanvasSize.y / KCDefine.B_VAL_2_FLT) + CSceneManager.UpOffset, KCDefine.B_VAL_0_FLT);
				} else {
					a_stPos = new Vector3(KCDefine.B_VAL_0_FLT, (CSceneManager.CanvasSize.y / -KCDefine.B_VAL_2_FLT) + CSceneManager.DownOffset, KCDefine.B_VAL_0_FLT);
				}
			}
			// 왼쪽, 오른쪽 UI 일 경우
			else if(bIsLeftUIs || bIsRightUIs) {
				a_stSize = new Vector3(KCDefine.B_VAL_0_FLT, KCDefine.B_SCREEN_HEIGHT, KCDefine.B_VAL_0_FLT);
				a_stPivot = bIsLeftUIs ? KCDefine.B_ANCHOR_MID_LEFT : KCDefine.B_ANCHOR_MID_RIGHT;

				// 왼쪽 UI 일 경우
				if(bIsLeftUIs) {
					a_stPos = new Vector3((CSceneManager.CanvasSize.x / -KCDefine.B_VAL_2_FLT) + CSceneManager.LeftOffset, KCDefine.B_VAL_0_FLT, KCDefine.B_VAL_0_FLT);
				} else {
					a_stPos = new Vector3((CSceneManager.CanvasSize.x / KCDefine.B_VAL_2_FLT) + CSceneManager.RightOffset, KCDefine.B_VAL_0_FLT, KCDefine.B_VAL_0_FLT);
				}
			}
		}
	}
	...
	/** 코너 고정 UI 상태를 설정한다 */
	private void SetupCornerAnchorUIsState(GameObject a_oUIsObjs, ref Vector3 a_stPos, ref Vector3 a_stSize, ref Vector3 a_stPivot, ref Vector3 a_stAnchorMin, ref Vector3 a_stAnchorMax, bool a_bIsEnableAssert = true) {
		CAccess.Assert(!a_bIsEnableAssert || a_oUIsObjs != null);
		...
		if(a_oUIsObjs != null) {
			...
			if(bIsUpLeftUIs || bIsUpRightUIs) {
				a_stSize = new Vector3(CSceneManager.CanvasSize.x, KCDefine.B_VAL_0_FLT, KCDefine.B_VAL_0_FLT);
				a_stPivot = bIsUpLeftUIs ? KCDefine.B_ANCHOR_UP_LEFT : KCDefine.B_ANCHOR_UP_RIGHT;
				a_stAnchorMin = bIsUpLeftUIs ? KCDefine.B_ANCHOR_UP_LEFT : KCDefine.B_ANCHOR_UP_RIGHT;
				a_stAnchorMax = bIsUpLeftUIs ? KCDefine.B_ANCHOR_UP_LEFT : KCDefine.B_ANCHOR_UP_RIGHT;
				
				// 좌상단 UI 일 경우
				if(bIsUpLeftUIs) {
					a_stPos = new Vector3(CSceneManager.LeftOffset, CSceneManager.UpOffset, KCDefine.B_VAL_0_FLT);
				} else {
					a_stPos = new Vector3(CSceneManager.RightOffset, CSceneManager.UpOffset, KCDefine.B_VAL_0_FLT);
				}
			}
			// 좌하단, 우하단 UI 일 경우
			else if(bIsDownLeftUIs || bIsDownRightUIs) {
				a_stSize = new Vector3(KCDefine.B_VAL_0_FLT, KCDefine.B_SCREEN_HEIGHT, KCDefine.B_VAL_0_FLT);
				a_stPivot = bIsDownLeftUIs ? KCDefine.B_ANCHOR_DOWN_LEFT : KCDefine.B_ANCHOR_DOWN_RIGHT;
				a_stAnchorMin = bIsDownLeftUIs ? KCDefine.B_ANCHOR_DOWN_LEFT : KCDefine.B_ANCHOR_DOWN_RIGHT;
				a_stAnchorMax = bIsDownLeftUIs ? KCDefine.B_ANCHOR_DOWN_LEFT : KCDefine.B_ANCHOR_DOWN_RIGHT;

				// 좌하단 UI 일 경우
				if(bIsDownLeftUIs) {
					a_stPos = new Vector3(CSceneManager.LeftOffset, CSceneManager.DownOffset, KCDefine.B_VAL_0_FLT);
				} else {
					a_stPos = new Vector3(CSceneManager.RightOffset, CSceneManager.DownOffset, KCDefine.B_VAL_0_FLT);
				}
			}
		}
	}
	...
	/** 유동 UI 상태를 설정한다 */
	private void SetupNonAnchorUIsState(GameObject a_oUIsObjs, ref Vector3 a_stPos, ref Vector3 a_stSize, ref Vector3 a_stAnchorMin, ref Vector3 a_stAnchorMax, bool a_bIsEnableAssert = true) {
		CAccess.Assert(!a_bIsEnableAssert || a_oUIsObjs != null);
		...
		if(a_oUIsObjs != null) {
			...
			if(a_oUIsObjs.name.Equals(KCDefine.U_OBJ_N_SCENE_PIVOT_UIS)) {
				a_stPos = new Vector3(KCDefine.B_SCREEN_WIDTH / -KCDefine.B_VAL_2_FLT, KCDefine.B_SCREEN_HEIGHT / -KCDefine.B_VAL_2_FLT, KCDefine.B_VAL_0_FLT);
			}
			// 크기 보정 가능한 UI 일 경우
			else if(bIsBaseUIs || bIsEnableCorrectUIs) {
				a_stSize = bIsBaseUIs ? KCDefine.B_SCREEN_SIZE : CSceneManager.CanvasSize;

				// 블라인드, 고정, 코너, 디버그 UI 일 경우
				if(bIsBlindUIs || bIsAnchorUIs || bIsCornerAnchorUIs || bIsDebugUIs) {
					a_stSize = Vector3.zero;
					a_stAnchorMin = KCDefine.B_ANCHOR_DOWN_LEFT;
					a_stAnchorMax = KCDefine.B_ANCHOR_UP_RIGHT;
				}
			}
			// 화면 UI 가 아닐 경우
	...
	private void SetupCameraData(UniversalAdditionalCameraData a_oCameraData, bool a_bIsEnableAssert = true) {
		...
		if(a_oCameraData != null) {
			...
			switch(CAccess.QualityLevel) {
				case EQualityLevel.HIGH: {
					a_oCameraData.antialiasing = AntialiasingMode.SubpixelMorphologicalAntiAliasing;
				} break;
				case EQualityLevel.ULTRA: {
					a_oCameraData.antialiasing = AntialiasingMode.SubpixelMorphologicalAntiAliasing;
				} break;
